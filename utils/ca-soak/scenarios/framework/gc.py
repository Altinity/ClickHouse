...
The README requires that every scenario drive `SYSTEM CONTENT ADDRESSED GARBAGE COLLECTION ca`
explicitly at checkpoints — even when background GC is enabled — so a report can separate workload
cost from reclamation cost. `SYSTEM CONTENT ADDRESSED GARBAGE COLLECTION ca` runs ONE synchronous GC
...
GC_SQL = "SYSTEM CONTENT ADDRESSED GARBAGE COLLECTION {disk}"
...
def forced_gc_to_fixpoint(cluster, unreachable_fn, *, max_seconds: float = 240.0, stable: int = 3,
    ...
    reclaims cleanly. We issue ONE explicit single-node `SYSTEM CONTENT ADDRESSED GARBAGE COLLECTION`
