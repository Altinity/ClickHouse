...
def test_render_twice_with_different_value_replaces_not_duplicates():
    cluster_boot.render_tuned_config({"cas_manifest_decode_cache_bytes": "1048576"})
    cluster_boot.render_tuned_config({"cas_manifest_decode_cache_bytes": "16777216"})
    for node in ("ch1", "ch2"):
        xml = _tuned_xml(node)
        assert xml.count("<cas_manifest_decode_cache_bytes>") == 1
        assert "<cas_manifest_decode_cache_bytes>16777216</cas_manifest_decode_cache_bytes>" in xml
        assert "1048576" not in xml


def test_render_overrides_a_key_already_present_in_the_base_xml():
    # cas_gc_interval_sec is already set (to 10) in the base storage_conf_ch{1,2}.xml — this exercises the
    # "replace a same-named child" branch directly, not just the "re-render from scratch" idempotency
    # covered above.
    cluster_boot.render_tuned_config({"cas_gc_interval_sec": "3"})
    for node in ("ch1", "ch2"):
        xml = _tuned_xml(node)
        assert xml.count("<cas_gc_interval_sec>") == 1
        assert "<cas_gc_interval_sec>3</cas_gc_interval_sec>" in xml
        assert "<cas_gc_interval_sec>10</cas_gc_interval_sec>" not in xml
