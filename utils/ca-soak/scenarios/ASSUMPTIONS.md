...
- **S24** small dedup-cache capacity — needs a storage_conf variant with a tiny `dedup_cache_bytes`
...
`SYSTEM CONTENT ADDRESSED GARBAGE COLLECTION ca` runs one synchronous round on the node it hits (only
