...
///
/// REQUIRES a read-only-opened disk, same as fsck/ca-gc-dryrun: this tool must never claim the live
/// server's mount (a second live mounter racing the real GC's lease/writes is exactly the split-brain
/// class the protocol is designed to prevent). Unlike fsck/ca-gc-dryrun, rebuildBaseline DOES write
...
public:
    ...
    void executeImpl(const CommandLineOptions & options, DisksClient & client) override
    {
        ...
        if (!dos)
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "ca-gc-rebuild: '{}' is not an object-storage disk", disk->getName());

        auto * ca = dynamic_cast<ContentAddressedMetadataStorage *>(dos->getMetadataStorage().get());
        if (!ca)
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "ca-gc-rebuild: disk '{}' is not content-addressed", disk->getName());

        if (!ca->isReadOnly())
            throw Exception(
                ErrorCodes::BAD_ARGUMENTS,
                "ca-gc-rebuild: open the CA disk read-only (<readonly>true</readonly>) — this tool must never "
        ...
        if (!rep.performed)
        {
            ...
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "ca-gc-rebuild: refused: {}", rep.refusal);
        }
    }
};
