...
| `--login[=<mode>]`       | Authenticate via OAuth2. Bare `--login` triggers ClickHouse Cloud auto-login. `--login=browser` or `--login=device` runs the Authorization Code or Device flow against any OpenID Connect provider; pair with `--oauth-url`, `--oauth-client-id`, and (per IdP) `--oauth-audience`, `--oauth-client-secret`, `--oauth-callback-port`. The same fields can be set in the connection block — see [OAuth login](./cli-oauth-login.md). | -                                                                                                                |
