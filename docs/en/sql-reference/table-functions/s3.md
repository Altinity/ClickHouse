...
Role-based access for S3 in ClickHouse Cloud is documented [here](/docs/cloud/security/secure-s3#access-your-s3-bucket-with-the-clickhouseaccess-role).
...
- [s3_truncate_on_insert](/docs/operations/settings/settings.md#s3_truncate_on_insert) - allows to truncate file before insert into it. Disabled by default.
- [s3_create_new_file_on_insert](/docs/operations/settings/settings.md#s3_create_new_file_on_insert) - allows to create a new file on each insert if format has suffix. Disabled by default.
- [s3_skip_empty_files](/docs/operations/settings/settings.md#s3_skip_empty_files) - allows to skip empty files while reading. Enabled by default.
