---- MODULE CaIncarnationCore_TTrace_1785649541 ----
...
_inv ==
    ~(
        ...
        gcRound = (1)
        /\
        man = ((s1 :> [fence |-> 1, refs |-> {}, log |-> <<[h |-> h1, op |-> "add"], [h |-> h1, op |-> "rem"], [op |-> "fence"]>>]))
        /\
        creator = ((h1 :> "none"))
        /\
        hbSeq = ((w1 :> 0 @@ w2 :> 0))
...
\* to `JsonSerialize`. For example, a sub-sequence of _TETrace.
    \* ASSUME
    \*     LET J == INSTANCE Json
    \*         IN J!JsonSerialize("CaIncarnationCore_TTrace_1785649541.json", _TETrace)
...
\*
\*trace == IODeserialize("CaIncarnationCore_TTrace_1785649541.bin", TRUE)
\*
\*=============================================================================
\*
...
trace == 
    ...
    ([cursor |-> (s1 :> 2),hbObs |-> (w1 :> -1 @@ w2 :> -1),fgRefs |-> (s1 :> {}),deadTok |-> (h1 :> {1}),fencePos |-> (s1 :> 3),trimBase |-> (s1 :> 0),wEv |-> (w1 :> {} @@ w2 :> {}),rootEdges |-> {},wView |-> (w1 :> 1 @@ w2 :> 0),nextTok |-> (h1 :> 3),fgCut |-> (s1 :> 0),roundOf |-> (L1 :> 1),tokOf |-> (h1 :> 1),reg |-> [man |-> (s1 :> TRUE), fence |-> 0, floor |-> (s1 :> 0), ns |-> {s1}, univ |-> (L1 :> {s1}), done |-> (L1 :> TRUE)],gcPhase |-> (L1 :> "fenced"),pendCasc |-> {},retired |-> {[t |-> 1, h |-> h1, r |-> 1]},gcRound |-> 1,man |-> (s1 :> [fence |-> 1, refs |-> {}, log |-> <<[h |-> h1, op |-> "add"], [h |-> h1, op |-> "rem"], [op |-> "fence"]>>]),creator |-> (h1 :> "none"),hbSeq |-> (w1 :> 0 @@ w2 :> 0),everEdged |-> {h1},treeEdges |-> {},fgPhase |-> "idle",inflight |-> {},marker |-> {},wDeps |-> (w1 :> {<<h1, 1>>} @@ w2 :> {}),fencedSet |-> (L1 :> {s1}),hbAlive |-> (w1 :> FALSE @@ w2 :> FALSE),present |-> (h1 :> TRUE),fgSeen |-> {},wedged |-> (w1 :> FALSE @@ w2 :> FALSE)])
    >>
----
...
---- CONFIG CaIncarnationCore_TTrace_1785649541 ----
CONSTANTS
    Writers = { w1 , w2 }
    Leaders = { L1 }
    Shards = { s1 }
