#!/usr/bin/env bash
# Run the complete `CaEdgeBeforeObserve` battery and assert every result by exact invariant name.
...
# Sabotages first: the reduced positive gate is evidence only after each removed guard is seen red.
# name                    expectation(green|violation)  expected-invariant
CONFIGS=(
    "sab_late_edge          violation  INV_NO_DANGLE"
    "sab_no_adopt_check     violation  INV_NO_DANGLE"
    "sab_no_k3_adopt_check  violation  INV_NO_DANGLE"
    "sab_no_k3_head         violation  INV_NO_DANGLE"
    "reduced                green      -"
)
...
do
    ...
    timeout "${TLC_TIMEOUT:-3600}" /usr/bin/java -XX:+UseParallelGC ${TLC_JAVA_OPTS:-} -cp "$JAR" tlc2.TLC \
        -metadir "$meta" -workers "${TLC_WORKERS:-1}" -deadlock -config "$cfg" "$MODULE.tla" >"$log" 2>&1
    rc=$?
