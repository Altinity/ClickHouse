#!/usr/bin/env bash
# Run one raw-body Gate-B config. Usage: ./run_metaraw.sh CaMetaDescriptorRaw_<cfg>
set -uo pipefail
cd "$(dirname "$0")"
CFG="$1"; LOG="../../../tmp/tlc_${CFG}.log"
/usr/bin/java -XX:+UseParallelGC ${TLC_JAVA_OPTS:-} -cp ../../../tmp/tla2tools.jar tlc2.TLC \
  -metadir ../../../tmp/tlc-meta -workers auto -deadlock -config "${CFG}.cfg" CaMetaDescriptorRaw.tla >"$LOG" 2>&1 || true
