...
(* Retired-list-inside-the-run settlement gate (spec 2026-07-10-cas-retired-in-snapshot §6).
   ...
   FoldAbort (deposed leader, spec §4/§6) IS modeled: a leader that performs the round's pre-CAS
   side effects — the exact-token redelete (durable prior token, same delExec as FoldRound) and the
   advisory `.meta` writes — but loses adoption (adopted/artifacts/lenAtAdopt unchanged) and only
   bumps nextAttempt so the next real round uses fresh keys. Its meta/delete effects genuinely
   persist into subsequent states — the point is to prove they are harmless: meta stays ADVISORY (no
   destructive transition reads it; a stray Condemned only causes an unnecessary resurrect, the safe
   direction) and a delete keys only off the durably adopted run's exact token at d = 0.
...
Settle(pc, d, tokNow, newRound) ==
  ...
  ELSE IF pc.st = "cond"
    THEN IF pc.round < newRound \/ Sabotage = "no_pacing"
           ...
           ELSE pc  \* carried (rule 4); also the branch clamp suppression takes in reality by
                    \* forcing carry even at pc.round < newRound (suppress_destructive; out of scope)
...
(* FoldAbort = a DEPOSED leader (spec §4/§6). It executes the round's pre-CAS side effects — the
   exact-token redelete against prior-adopted delete_pending rows at d = 0, with the DURABLE prior
   token (the SAME delExec/delTok/Settle as FoldRound), and the advisory `.meta` writes for its
   computed condemned set — but loses the gc/state adoption CAS: its run artifact never lands (or
   lands as an orphan attempt-scoped object retention later prunes), so `adopted`, `artifacts`, and
   the coverage floor `lenAtAdopt` do NOT move. It still bumps `nextAttempt`, so the next real round
   mints fresh keys (attempt-pinning, §4). The point is that these effects PERSIST into later states:
   this proves spec §4/§6's load-bearing claim that a deposed leader's unadopted advisory meta +
   pre-CAS deletes are harmless — no destructive transition reads `meta` (a stray "cond" only causes
   an unnecessary resurrect, the safe direction), and a delete only fires against the durably-adopted
   run's exact token at d = 0 (the deleted blob's adopted.edges = 0 = journal truth, so INV_NO_LOSS's
   antecedent is false). Bounded by nextAttempt for a finite space. *)
FoldAbort ==
  /\ adopted.round < MaxRound
  /\ nextAttempt <= MaxRound
  /\ \E cut \in lenAtAdopt..Len(journal) :
     LET newRound == adopted.round + 1
         newEdges == [b \in Blobs |-> adopted.edges[b] + Delta(b, adopted.cut, cut)]
         newCond  == [b \in Blobs |-> Settle(adopted.cond[b], newEdges[b], phys[b], newRound)]
     ...
     IN /\ phys' = [b \in Blobs |-> IF delExec(b) THEN 0 ELSE phys[b]]
        /\ badDeleteEver' = (badDeleteEver
                             \/ \E b \in Blobs : delExec(b) /\ phys[b] /= adopted.cond[b].tok)
        (* Advisory meta from a DEPOSED (non-adopting) leader is ADD-ONLY: it may only set "cond"
           (the safe direction — a stray Condemned costs at most an unnecessary resurrect). It must
           NOT clear a blob to "clean" from an unadopted view: that would retract a still-condemned
           blob's freshness gate and let a writer reuse the live-but-condemned exact token, which a
           later pending redelete then kills (INV_NO_LOSS). Only an ADOPTED FoldRound whose spare is
           authoritative may clear to "clean". In reality meta transitions are etag-CAS-guarded and a
           deposed leader's spare is never durable. *)
        /\ meta' = [b \in Blobs |-> IF newCond[b].st \in {"cond", "pend"} THEN "cond" ELSE "clean"]
        /\ nextAttempt' = nextAttempt + 1
        /\ UNCHANGED <<journal, liveRef, nextTok, adopted, artifacts, lenAtAdopt>>
