#!/usr/bin/env bash
# Run the complete `CaGcAckFloorZombie` battery and assert every result by exact invariant name.
...
# Sabotage first: a green result is evidence only after eager zombie deletion is seen red.
# name               expectation(green|violation)  expected-invariant
CONFIGS=(
    "sab_eagerdelete  violation  INV_NO_DANGLE"
    "stage1          green      -"
    "empty_blobs     green      -"
