...
**Verdict for Task 6:** the P3.1 mount-lease fix + F1 fix are **validated** — fence-recovery PASS (manual cycle + 40-round S13 chaos, epoch 26, no wedge) and S13 dev-scale 11/12 (all safety green). The full-scale run is infra-limited (F2), not a CAS defect; re-running at fitted scale for a clean at-scale verdict. Marking the P1 backlog entry RESOLVED on this evidence.
