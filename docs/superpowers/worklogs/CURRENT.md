...
2026-07-30 11:5x — TASK 1B CLOSED. Deletion round eeb194af24b verified by me inline: comments-only proven mechanically (comment-stripped trees hash-identical per file), zero line-number citations left (carve-time caps now cited by SYMBOL), and every surviving sentence checked for strengthening — none. Four commits, gate 1585/245, reviewed twice with ZERO code/test defects both times. Next: Task 1c.
2026-07-30 12:0x — TASK 1C DISPATCHED (base a6d468c8fde, baseline 1585/245). Four risks restated in the dispatch: Step 0 goldens before any key change; listNamespaces' producer-surfaces-consumer-decides with decommission refusing fail-close; MINOR-B in the same audit; the DDL disposition re-opened for decision on the new evidence. Prose rules from the five-round predecessor included as hard constraints — cite symbols not line numbers, keep claims local or make them execute.

2026-07-30 — Task 1c landed `NamespaceLifeId` over refs AND namespace files (gate 1601/246, six commits, `90221e634ab..307a0227fb9`), and its own red-first evidence carried the honest half: three type asserts and two Constraint-12 fences did not fail before the change, disclosed as "they fence, they do not fix". Two decisions went against the brief's letter and were disclosed rather than buried — the MINOR-B guard only MOVED the crash 200 lines into shared `recoverRefTable`, so a second guard went there with an equivalence argument, and the DDL consumer's ruling was REVERSED to record-and-continue because refusing would make the obstacle permanent and blocked by the one operation that clears it. Review dispatched with both as explicit checks, prose surface first.

2026-07-30 — Task 1c fix round 1 came back CLEAN on all six items, and the re-review earned that verdict by checking the code rather than the account of it. Then, verifying the implementer's own named follow-up ("no fence exists for the exit set"), I found the same defect one surface further out: `lifeless_keys` never reaches `contentAddressedFsckColumns` — the SQL row for `SYSTEM ... FSCK` — even though that function's own comment carries the rule "EVERY TERM OF `FsckReport::clean` APPEARS HERE ... added here in the same change". Fourth recurrence of one shape in this file family. And the reason it keeps recurring was sitting next to it: the summary-line fence's comment claims it is "written against `clean()`'s OWN field set rather than a hand-listed set of names, so that adding a hard finding ... fails HERE" — its body is five hand-listed names, so the mechanism does not exist, and the missing SQL column is the proof. Round 2 ordered the rule turned into something that EXECUTES, red-first, and forbidden from claiming more than it checks.

2026-07-30 (afternoon) — Two process changes, both from measurement rather than taste. First, non-code findings stopped entering fix rounds: Task 1c had spent THREE consecutive rounds on comment and claim accuracy with zero defects in its code or tests, so `docs/superpowers/cas/deferred-docs-fixes.md` now batches them, and classifying the six deferred "minors" for it found only three were prose — the others were a vestigial guard, a loose LIST prefix, and a test whose assertion passed even when the property its comment named was violated. Second, the `LOGICAL_ERROR`-under-sanitizer abort class got a mandatory review trigger on its FIFTH recurrence in a week, with a file-wide grep sweep, because the abort hides every test after it — and the round that fixed it demonstrated the mechanism by growing the surface from 6 to 10 expectations before the sweep ran. Task 2 (the catalog object) closed: spec PASS, one fix round, re-review CLEAN, gate 1639/249. Its most valuable finding was a silent default substitution — the retry path treated a vanished catalog as an empty one and would have replaced every namespace's catalog with a single mutation's output via create-if-absent; the in-tree analogue `PoolMeta::admitOrValidate` already fails closed on exactly that observation. And the `05008` blocker was NOT our regression: both of my hypotheses (key-shape divergence, watermark floor) were refuted, and the cause is `UniversePolicy::kDefault == StageA_Suppressed` making destruction suppressed on every production round — introduced by a commit that swept 20+ gtest files and touched zero files under `tests/queries/`. Two more stateless tests were predicted red by the same mechanism and confirmed by running them unmodified first. The restoration marker now lives at the line Task 7b will edit, not only in the test files, because a marker only in the test is how this miss happened.

2026-07-30 (evening) — Task 4 turned out to be Tasks 2+3 combined and was split; 4-A (format bump B) and 4-B (the map) landed, and 4-C — production birth wiring plus the catalog-authoritative universe — is in its second round of review fixes. The arc's real subject was one defect asked three times. R11: the destructive gate compares `proven == total`, and with an empty universe that is `0 == 0`, so a fresh pool reads as fully proven. R11b, from asking whether the guard's predicate was right: the denominator is the UNION of hint, sealed cursors and catalog, so `frontier_namespaces > 0` refuses only a fresh pool and never the DAMAGED catalog the finding named — and worse, before the change a damaged pool would have been walked where its objects actually are, so moving the authority onto one object made that object's absence indistinguishable from "everything is proven empty". R11c, from asking the same question of the FIXED code: the guard keys on the catalog not naming a namespace at all, and does nothing when it names it at an incarnation whose key space is empty, while the R10 filter drops the real incarnation's keys silently by design. Unreachable today, so it gates Task 7b rather than sitting in a ledger — two later tasks would each make it live. The transferable rule: when a gate rests on `a == b`, ask what happens when both are zero, then ask where each side CAME FROM. And a process lesson with teeth: the R11b ruling asked for one explicit predicate about the authority; what shipped kept the predicate and changed the numerator's source, which is equivalent today but leaves an invariant implicit across three sites — three places to stay silent where an explicit predicate has one place to refuse, and that is exactly the gap R11c walked through. Elsewhere: the gate's suite list was found to omit 17 CAS suites (fourth recurrence, and the correct filter had been in memory for nine days without reaching the tool — now fixed at the generator, which derives from sources and fails on any unclaimed suite); four more tests were found asserting defects as correct, two of them pinning data loss; a uniform mechanical pin inverted the one test whose premise was the ABSENCE of the pinned thing; and the pinned TLC jar was proved to report a violation for `<> TRUE`, so three temporal expectations may have been green for the wrong reason since they were written.
