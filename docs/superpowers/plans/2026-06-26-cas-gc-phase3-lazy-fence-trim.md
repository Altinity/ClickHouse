...
**Architecture:** `Gc::trim` (`CasGc.cpp:168`) erases journal records `at_version <= cursor` for every shard. Today `cursor` comes from a transient in-memory snap cursor; Phase 3 re-sources it from the **sealed** per-`(ns,shard)` coverage (`CasFoldSeal.per_ns_shard.ShardCoverage.folded_cursor`) so trim runs only below what a durable generation actually incorporated — never above it (`INV_JOURNAL_COVERAGE`) — and records the trim cursors it used into the `CasCompletionSeal` (the round/adoption visibility seal). `Gc::fence` (`CasGc.cpp:819`) is **untouched**: it still fences the registry then **every** root shard of every namespace in the fence-time registry, recording `state.fence_version[round][cursorKey(ns, shard)]`. The fence is ALWAYS global; a real lazy fence is deferred (see [Deferred / Out of scope](#deferred--out-of-scope)) because it needs a proven atomic writer-visible fence authority that does not yet exist; until then GC fences every shard.
...
**Source spec:** `docs/superpowers/specs/2026-06-26-cas-gc-streaming-sharded-redesign-design.md` (rev. 15), §Global Fence, §Trim, §Phase Plan/Phase 3. The governing rule: *every shard in the fence universe is fenced every round (lazy fence is NOT implemented); trim only below sealed generation coverage.*
...
- **Trim coverage (spec §Trim, `INV_JOURNAL_COVERAGE`).** Trim removes journal records only below the **sealed** per-`(ns,shard)` cursor coverage carried in `CasFoldSeal` (the canonical `per_ns_shard.ShardCoverage` produced by Phase 1d, persisted by Phase 2), and records the trim cursors it used into `CasCompletionSeal.trim_cursors`. Every shard is fenced every round, but trim is still gated on the sealed coverage cursor — never above it. This is the existing `at_version <= cursor` rule (`CasGc.cpp:168`) re-sourced from the sealed `CasFoldSeal` coverage rather than from a transient in-memory snap cursor.
...
- `CasFoldSeal` (write-once `gc/gen/<gen>/fold_seal`, `FormatId::FoldSeal` = "CAFS") carries `per_ns_shard` entries of type `ShardCoverage` (classification, folded token, folded cursor). These are produced by Phase 1d and persisted across rounds by Phase 2. Trim reads `ShardCoverage.folded_cursor`.
- `CasCompletionSeal` (write-once `gc/gen/<gen>/completion_seal`, `FormatId::CompletionSeal` = "CACS") carries `fence_positions`, `delete_outcomes`, `trim_cursors`, and `adoptable` — the round/adoption visibility seal. Trim records the cursors it used into `CasCompletionSeal.trim_cursors`.
...
Phase 3 introduces **no** contract extension: lazy fence is dropped (the fence is always global), so there is no reuse-justification field. Only these contract type names appear in code: `CasFoldSeal`, `CasCompletionSeal`, `ShardCoverage`, `CasGc`.
...
- **Canonical contract only:** the only contract type names used are `CasFoldSeal` (its `per_ns_shard.ShardCoverage.folded_cursor` that `trim` reads), `CasCompletionSeal` (its `trim_cursors` that `trim` records into), and `CasGc` `trim`; Phase 3 adds **no** new field or type (lazy fence dropped, the fence is always global ⇒ no reuse-justification field). No parallel type is invented, and the retired single `CasGenerationSeal` is not referenced. ✓
