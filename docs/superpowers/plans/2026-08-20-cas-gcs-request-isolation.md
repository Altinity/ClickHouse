...
1. Default `gcs_hmac`: HEAD, GET, LIST, CopyObject, DELETE, batch `DeleteObjects` and multipart. Assert preserved response ETags, no generation precondition, accepted existing selector/credential spelling, and typed errors. Checksum-required and chunked uploads are NOT included: the only producer of those headers is the S3 Express path, selected from an endpoint containing `s3express`, which cannot also be `storage.googleapis.com`. User metadata on an ordinary write is not included either, since no production caller passes object attributes. Both are pinned by unit tests against the request object instead.
2. NativeConditional `gcp_oauth`: conditional PUT, native-token HEAD, and exact DELETE. Assert GCS accepts each request, the wire uses `x-goog-if-generation-match`, the response token is the created generation, and stale AWS signing artifacts (`x-amz-date`, `x-amz-content-sha256`, `x-amz-security-token`, `x-amz-api-version`) are absent. The checksum-bearing and chunked/framed variants are unreachable for the reason given in group 1, and the attribute round trip has no production producer; the unit tests own both.
