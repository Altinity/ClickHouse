...
**Body shape:** `cas_gc_outcomes` is `Control` in role but carries a repeated entry list, and its traits give it a 64 KiB **line** cap + 256 MiB object cap — so it is **line-structured**, materialized whole: header line + one flat JSON record per `OutcomeEntry` (insertion order, matching today's encoder) + a `{"n":<count>}` trailer. `Always` compression → the stored bytes are one zstd frame (`.zst` key). NOT deterministic (idempotent `putIfAbsent`; a replay re-decodes and semantically verifies — it never byte-compares, which zstd would defeat anyway), so insertion order is fine.
