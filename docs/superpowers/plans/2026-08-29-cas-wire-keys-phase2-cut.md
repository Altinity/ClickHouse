# CAS Wire Keys Phase 2: Atomic Cut Implementation Plan
...
- **Docs ride each commit (spec rule).** Every format task updates, in the SAME commit as its key flip: the codec's own header/impl comments that spell old keys, and that format's row/example in `Formats/README.md`. Task 19 is a final docs AUDIT, not the place docs land.
...
- [ ] **Step 3: Re-stamp fixtures (raw AND escaped).** Sweep both source forms for every version above 1: `rg -n '"v":([2-9]|10)' src/Disks/tests/` AND `rg -n '\\\\"v\\\\":([2-9]|10)' src/Disks/tests/` (known escaped homes: `gtest_cas_blob_meta_format.cpp:65-71`, `gtest_cas_gc_state_format.cpp:92,134,166`, `gtest_cas_server_root_format.cpp:141`, `gtest_cas_gc_outcomes_format.cpp:121-137`, `gtest_cas_text_format.cpp:218,260,266`). Classify every hit: (a) historical refusal → deleted in Step 2; (b) the `v:2` forward-gate fixture → stays; (c) a body-under-test fixture → restamp to `"v":1` and update its "any version <= G_BUILD" comment. Finish with a zero-hit check for versions 3–10 in both forms.
...
**Files:** Modify `Formats/CasFoldSealFormat.{h,cpp}`, `src/Disks/tests/gtest_cas_fold_seal_format.cpp`, the hold-grammar suite, battery golden, reservation tests, `Formats/README.md` row.
...
- [ ] **Step 1: Worklist is type-directed.** `grep -rn "\.classification\|classification ==\|classification =" src/Disks/` — the full result across production AND tests is the migration list (the line numbers above are the review-time snapshot, the grep is the authority). `enum class` accepts no integer literals: every initializer, assignment, comparison (`== 0`, `== 4`), and message becomes a named `CoverageClass` value. The old byte 4 belongs to the numeric wire being deleted (nothing outside this JSON persists the raw byte) — `Clamped = 3` keeps the table dense.
- [ ] **Step 2:** Wire: key `cls` → `class`, value becomes the word (`fromWord` → `CORRUPTED_DATA` on unknown). Hold grammar re-expressed: hold present iff `Clamped`. `CasInspect` renders via `coverageClassToWord`; pin all four words in `gtest_cas_inspect.cpp`. Update goldens and the closed-set negative.
- [ ] **Step 3: Wording pass (carry-forward):** messages naming dead spellings move to the live vocabulary — `binding missing bk/rn` names `old_kind`/`old_ref` (or the bundle words); sweep `grep -rn 'bk/rn\|me/mb/mo\|ha/h\|tt/tv' src/Disks/DiskObjectStorage/MetadataStorages/ContentAddressed/` and update every affected pinned expectation, naming each.
...
- [ ] **Step 2: External sweep — all three roots, raw AND escaped forms.** Sweep `tests/integration/` (whole tree), `utils/ca-soak/` (whole tree), and ALL CAS stateless tests (`tests/queries/0_stateless/*cas*` — not just 05010/05012) for every spelling ANY task of this plan flips (this is the whole-plan external checkpoint, not just catalog keys). Known hits: the `t8_s44` regex `\{"k":"ent","ns":…,"st":…,"inc":…`; `05023`'s catalog-row parser. Fix each; integration/soak lanes do not run in the unit gate — list every touched external file in the commit body; phase 3 executes them.
...
**Files:** Modify the codec headers/impls that today expose only render delegates (add the narrow PARSE delegate beside each: known gaps include `CasBlobEnvelopeFormat.h` (~35-36) and `CasRefLogFormat.h` (~44-46); audit every table for a public `fromWord` delegate — outcomes, meta-state, placement, run marker, hold reason included), `src/Disks/tests/gtest_cas_wire_vocab.cpp` + per-codec test files for the value-set pins, `Formats/README.md`, `docs/superpowers/cas/BACKLOG.md`.
...
**Files:** none created except the report: `.superpowers/sdd/2026-08-29-cas-wire-keys-phase2-cut/task-20-report.md`.

- [ ] **Step 1: Full gate** green, logged to `build/test_cas_phase2_gate.log` (subagent-analyzed).
- [ ] **Step 2: Dead-spelling sweeps — five separate source-aware inventories** over `ContentAddressed/`, `src/Disks/tests/`, `tests/integration/`, `utils/ca-soak/`, and `tests/queries/0_stateless/` (CAS tests):
  1. **Carrier literals:** `rg -n 'WireKey\{"(ha|h|tt|tv|me|mb|mo|ome|omb|omo|nme|nmb|nmo|obk|orn|nbk|nrn|st|cr|sz|il|hln|gcs|mrg|alg|rnd|sg|spt|sa|msc|lo|ls|nwe|su|rt|hn|sat|eat|ma|fen|le|cte|cts|cse|css|lse|lss|we|rs|lc|rn|ts|pd|pm|b|s|m|pend|mc|oc|g|pg|ck|cls|lfe|lfs|hr|hpe|hps|hrc|hnr|ct|pt|ocr|bld|by|ch|inc|rsr|csr|cwe|cfg|cur)"\}' ContentAddressed/` → zero hits (pool `pid` flipped to `pool_id`; mount `pid`/`seq`/`write_attempt_id` are LIVE and excluded).
  2. **Raw JSON keys:** the same alternation as `'"X":'` over all five roots.
  3. **Escaped JSON keys:** the same alternation as `'\\"X\\":'` (C++ string sources).
  4. **Critical keys and tag values:** `'!pse|!pss'` in both forms; tag VALUES `'"(ent|rfl|btr|cnd)"'` in both forms; the snapshot tags via `'(kind|k)\\?":\\?"(c|p)"'` variants.
  5. **Old value representations:** a comma inside an `algos_used` string value; numeric `"class"`/`cls` values.
  Legal hits, and nothing else: the sentinel guards (`pl`/`rte`/`rts` literals and their tests), Task 18's marked baseline literals, the non-alias negative fixtures, framing (`type`/`v`/`n`, run-header `kind`), and live mount-lease `pid`/`seq`/`write_attempt_id`. Anything else is an unflipped spelling: fix and re-gate.
