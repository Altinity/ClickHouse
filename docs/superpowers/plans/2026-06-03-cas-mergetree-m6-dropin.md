...
- [ ] B33: extend `MergeTreeData::checkContentAddressedDiskRestrictions` (or an engine-level check) to reject `Replicated*MergeTree` on a CA disk with a clear message. Test (stateless): `CREATE TABLE … ENGINE=ReplicatedMergeTree(...)` on a CA disk throws.
- [ ] B34: reject the BACKUP hard-link path on CA with a clean message (or verify+document pointer-holding BACKUP). Minimal: a clear error, not a raw `LOGICAL_ERROR`.
- [ ] Config: create `tests/config/config.d/content_addressed_storage_policy_for_merge_tree_by_default.xml` mirroring `s3_storage_policy_for_merge_tree_by_default.xml` — a `content_addressed` disk over `object_storage_type=local` (fast variant) as the **default** MergeTree policy, with `content_addressed_gc_enabled=1`, `content_addressed_gc_grace_sec=60`, `content_addressed_gc_interval_sec=5`, and a real local `cas_scratch` path.
- [ ] Investigate how the `s3 storage` stateless variant is wired (`ci/jobs/functional_tests.py` `--options` → which config.d are installed by `tests/config/install.sh`) and add a `content_addressed storage` option that installs the new config (so the suite runs with CA-as-default without affecting other variants).
- [ ] Smoke: run ~10 representative stateless tests (a simple SELECT/INSERT test, an aggregation, a join, a mutation test, an ALTER test) under CA-default; confirm the config mounts and capture the first failure shapes. Build clean; existing CA unit (59) + stateless (04278–04282) still green.
- [ ] Commit `CAS M6: B33/B34 gates + content_addressed-default stateless config + smoke`.
