# CAS carve script Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `utils/cas-carve/carve.sh` — a rerunnable bash script that reconstructs the essential fragment of the CAS branch as an ordered commit series on a fresh branch from `altinity/antalya-26.6`, with a hard completeness guarantee.

**Architecture:** One self-contained bash script: declarative data at the top (EXCLUDES + ordered GROUPS built via a `group` helper), a generic engine below (classification → `--check` report → worktree apply → completeness invariant). Classification is pure-bash `[[ == ]]` pattern matching (where `*` crosses `/`), first-match-wins, hard fail on unmatched files.

**Tech Stack:** bash, git (`diff --name-status`, `worktree`, `checkout <ref> -- <path>`).

**Spec:** `docs/superpowers/specs/2026-08-03-cas-branch-reconstruction-script-design.md`.

## Global Constraints

- The primary checkout is shared with live sessions: the script must never switch branches there; all branch construction happens in a temporary worktree `tmp/carve-worktree`.
- The script lives at `utils/cas-carve/carve.sh` and excludes itself from the carve.
- Defaults, verbatim from the spec: `--src HEAD`, `--base altinity/antalya-26.6`, `--branch cas-carved`.
- The new branch starts at `git merge-base $BASE $SRC` — never at the tip of `$BASE`. This is what makes future base advancement safe (see Task 3, spec §9 step).
- Generated reconstruction commits get plain messages (no AI trailer); commits that add/modify the script itself follow normal repo commit rules.
- EXCLUDES, verbatim from the spec: `docs/superpowers/`, `.superpowers/`, `utils/ca-soak/`, `utils/cas-gate/`, `utils/cas-carve/`, `tmp/`, `.gitignore`, `tests/broken_tests.yaml`.
- Hard fail (exit 1) on: any diff file matching neither EXCLUDES nor any group; completeness invariant violation; existing target branch without `--force`.
- Warning only (not an error): a group that matches zero files.
- Never use `sleep`-style waiting; never push.

---

### Task 1: Engine with classification and `--check` mode

**Files:**
- Create: `utils/cas-carve/carve.sh`

**Interfaces:**
- Produces: the `group <name> <pattern>...  <<'MSG' ... MSG` declaration helper and pattern syntax that Task 2's manifest uses. Pattern syntax: plain bash `[[ == ]]` glob against the repo-relative path (`*` matches `/` too); optional status prefix `A:` (added-only) or `M:` (modified-only). Arrays: `GROUP_ORDER`, `GROUP_PATTERNS[name]` (newline-joined), `GROUP_MESSAGES[name]`.
- Produces: globals Task 3's apply phase uses: `SRC_SHA`, `MB`, `BRANCH`, `FORCE`, `DIFF_FILES` (diff-order file list), `FILE_STATUS[file]` (A/M/D), `CLAIM[file]` (group name or `__excluded__`).

- [ ] **Step 1: Write the script with the engine and a deliberately incomplete manifest**

Create `utils/cas-carve/carve.sh` (mode 755):
...
- [ ] **Step 2: Run `--check` and verify it fails with an unmatched list**

Run: `utils/cas-carve/carve.sh --check 2>&1 | tail -20; echo "exit=$?"`
Expected: the classification table prints (group `upstream-b115` claims 2 files, excluded ≈ 950+), then `ERROR: <N> files match neither ...` with a long list, `exit=1`. This proves the hard-fail exhaustiveness guard works before the real manifest exists.

- [ ] **Step 3: Commit**

```bash
git add utils/cas-carve/carve.sh
git commit -m "cas-carve: classification engine with --check mode
...
### Task 2: Full manifest (phases 1–6)

**Files:**
- Modify: `utils/cas-carve/carve.sh` — replace the placeholder manifest block (between the `group()` definition and the `# Engine: classification` banner) with the full ordered manifest below. Engine code is untouched.

**Interfaces:**
- Consumes: `group` helper and pattern syntax from Task 1.
- Produces: the complete `GROUP_ORDER` the apply phase (Task 3) commits in order.

- [ ] **Step 1: Replace the placeholder manifest with the full one**
...
- [ ] **Step 2: Run `--check` and verify full coverage**

Run: `utils/cas-carve/carve.sh --check`
Expected: exit 0, `--check OK`, no `ERROR`, per-group counts all > 0 (warnings acceptable only for `contrib/*`-style safety globs — currently `contrib` has no tracked changes, so `cas-wiring-registries-and-entrypoints` still matches via its other patterns; no group should be fully empty). Sanity-check the counts against the known shape: `cas-tests-unit` ≈ 134, `cas-formats` ≈ 40, `cas-pool` ≈ 31, excluded ≈ 900+.

- [ ] **Step 3: Cross-check phase-1 counts against the spec**

Run: `utils/cas-carve/carve.sh --check | head -40`
Verify each `upstream-*` group's count equals its pattern-list length (these are explicit files — a shortfall means a typo'd path, which the engine cannot distinguish from "file not in diff"). If any count is short, diff the group's list against `git diff --name-only $(git merge-base altinity/antalya-26.6 HEAD)..HEAD -- src` and fix the path.

- [ ] **Step 4: Commit**

```bash
git add utils/cas-carve/carve.sh
git commit -m "cas-carve: full manifest, phases 1-6
...
### Task 3: Apply phase, completeness invariant, spec maintenance note

**Files:**
- Modify: `utils/cas-carve/carve.sh` — replace the trailing `apply phase not implemented yet` stub with the apply phase below.
- Modify: `docs/superpowers/specs/2026-08-03-cas-branch-reconstruction-script-design.md` — append §9.

**Interfaces:**
- Consumes: `SRC_SHA`, `MB`, `BRANCH`, `FORCE`, `DIFF_FILES`, `FILE_STATUS`, `CLAIM`, `GROUP_ORDER`, `GROUP_MESSAGES`, `EXCLUDES`, `pattern_matches` from Tasks 1–2.

- [ ] **Step 1: Implement the apply phase**

Replace the two stub lines at the end of the script with:
...
- [ ] **Step 2: Full run and verify the invariant**

Run: `utils/cas-carve/carve.sh --branch cas-carved-test 2>&1 | tail -40`
Expected: exit 0, one summary line per non-empty group (~28 commits), the discard table, and `OK: completeness invariant holds`.

- [ ] **Step 3: Inspect the produced branch**

Run: `git log --oneline --reverse cas-carved-test | head -40` and `git log --stat cas-carved-test -3`
Expected: commits in manifest order (phase 1 first), sensible per-commit file lists. Then double-check the invariant independently of the script:
`git diff cas-carved-test HEAD --stat -- . ':!docs/superpowers' ':!.superpowers' ':!utils/ca-soak' ':!utils/cas-gate' ':!utils/cas-carve' ':!tmp' ':!.gitignore' ':!tests/broken_tests.yaml'`
Expected: empty output.

- [ ] **Step 4: Verify rerun behavior**

Run: `utils/cas-carve/carve.sh --branch cas-carved-test; echo "exit=$?"`
Expected: `ERROR: branch 'cas-carved-test' already exists`, `exit=1`.
Run: `utils/cas-carve/carve.sh --branch cas-carved-test --force 2>&1 | tail -3`
Expected: exit 0, invariant holds again. Then clean up: `git branch -D cas-carved-test`.

- [ ] **Step 5: Append §9 to the spec**

Append to `docs/superpowers/specs/2026-08-03-cas-branch-reconstruction-script-design.md`:
...
```bash
git add utils/cas-carve/carve.sh docs/superpowers/specs/2026-08-03-cas-branch-reconstruction-script-design.md
git commit -m "cas-carve: apply phase with completeness invariant; spec: maintenance section
