...
- **No migration.** Pre-GA on `cas-mergetree-poc`, never on prod. The snap codec is **reset to v1** of the cursor-carrying format and `gc/state.folded_cursor` is removed (`gc/state` codec bumped to v3). A stored old-format frame **fails closed** on decode (version mismatch → exception), never silently misread. Because the feature is pre-GA with no such objects on any target, a fresh pool simply cold-starts (full fold from cursor 0); an operator upgrading a scratch pool wipes `gc/`. No back-compat path, and an old cursor is never trusted for a trim or delete.
