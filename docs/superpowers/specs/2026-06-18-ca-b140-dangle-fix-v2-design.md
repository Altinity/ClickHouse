...
1. **`Core/CasGcSnap.{h,cpp}` — store the cursor in the snap.** Extend `GcSnap` with a per-shard `folded_cursor` field and serialize it in `encodeSnapFields`/`decodeSnapFields`. The feature is pre-GA and was never on prod, so there is **no migration**: reset the snap codec version to **v1** of the cursor-carrying format and drop all back-compat with the prior format (a stale/absent snap is simply rebuilt by a full fold from cursor 0 — the normal cold-start path). Byte-equal generation adoption (`putIfAbsent` + the `snap[g] == candidate` comparison in `fold`/`cascadeAndPersist`) then becomes **cursor-aware** automatically: a generation folded to a different cursor is not byte-equal, so a leader cannot adopt it as its own — it probes upward and writes its own coherent `(edges, cursor)`.
...
- **No migration.** The feature is pre-GA on `cas-mergetree-poc` and was never on prod, so there is no on-disk compat obligation. The snap codec version is **reset to v1** of the cursor-carrying format with no back-compat path. Any pre-existing `gc/snap` from the old format is treated as absent and rebuilt by a full fold from cursor 0 (the normal cold-start path) — fail-closed, never trusted for a trim or delete.

## 6. Tests (TDD)
- **FIRST deliverable — the failing gtest** (before the implementation plan): rewrite `src/Disks/tests/gtest_cas_b140_dangle.cpp` to the real shared-blob/cursor-skip shape — two `Gc` instances over one `Store` (the `gtest_cas_gc_round.cpp` template), a blob shared by `T_live` and a cross-node-published live `T_cur`, a path that makes the committed `folded_cursor` incoherent with the committed snap edges, then a trim + strip. This is the **empirical pin** that the dangle reproduces in the *real code* (not just the model) and the RED test the fix turns green: it must `EXPECT` `runFsck().dangling > 0` (live-blob deletion) on the current code, then flip to `== 0` after the fix. Prefer a **natural** 2-leader/lease-steal repro over byte-injection; a faithful injection of the real incoherent `(snap, folded_cursor)` state is an acceptable fallback if a natural interleaving cannot be driven through the public APIs.
