...
Memory is O(block) for the IO buffers plus O(1) per current blob — never the pool, never a blob's whole edge set held at once (edges pass one at a time). No random access, no full-set materialization, no second pass.
