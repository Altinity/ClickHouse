...
**Status:** DRAFT (2026-07-28) — spec only; no code landed. Companion plan:
`docs/superpowers/plans/2026-07-28-cas-fence-observability-and-write-grace.md`.
...
On the CA-over-S3 CI lanes the RustFS object store refused TCP connections in short bursts (fd
exhaustion, fixed separately). One burst overlapped the mount-lease renewal window: the
`MountLeaseKeeper` could not confirm its renewal inside the lease TTL (30 s default), so the mount
fence tripped. This is the **fence-not-rescue** design working as intended — a lease holder that
cannot confirm renewal is not live, so it fences and self-remounts; this is CORRECT and is not
weakened here. The episode (msan lane, sha `07f8398acddff2c`, core window 02:02:52→02:05:00 with
stragglers to 02:10) had two consequences:
...
**Correction to the brief.** Renewal failure is *not literally* unlogged. The base
`SingleWriterSlot::backgroundLoop` (`CasServerRoot.cpp:1176-1237`) already logs **both** the
transient-retry case (`:1202`, *"background renewal failed transiently, retrying while the lease is
still valid"*) and the terminal fence-stop case (`:1209`, *"background renewal failed, the
mount-lease stops advancing"*) via `tryLogCurrentException`, which defaults to **Error** level
(`Exception.h:276`). Both fire on the keeper's `CasMountLeaseKeeper` logger. So the underlying store
error *does* reach `err.log`.

What is genuinely invisible (verified as zero logging) is the part an operator actually needs to
reconstruct the episode — the **runtime-side fence arm and the entire self-remount lifecycle** — and
the coherence of the timeline:
...
- `CasSelfRemountMicroseconds` (`ValueType::Microseconds`): total wall-time spent
  fenced-and-remounting — incremented by the A6 fenced-window duration at the same completion site.
  `total / CasSelfRemountCompleted` is the average fence-window length, the single most useful number
  for "how long were writes refused".
...
- **Signalers** (all `write_grace_cv.notify_all()`), added at the existing transition sites, each a
  ...
  - `armMountFence` (`CasMountRuntime.cpp:133`) — the remount re-armed the fence ⇒ wake waiters;
    they observe `mayMutate()` true and return (proceed).
...
2. **The generation is captured AFTER the wait, so no stale incarnation rides through.** The
   ...
   *after* the wait returns, i.e. under whatever incarnation is current then. A write only proceeds
   ...
   two conditions as today. A remount that re-arms under a new incarnation is exactly the case the
   write now admits *into*, not rides *through*: it does its work under the new incarnation from the
   start, having done nothing under the old one (the wait is at the entry gate; §B.3 excludes the
   mid-operation re-checks).
