...
## Fix L1 — publish under the final part identity {#fix-identity}

Teach the part-path parser a `MOVING_DIR_NAME` reserved-directory case, mirroring the existing `detached` case: `store/<u3>/<uuid>/moving/<part>/<file>` → `(live namespace, ref = <part>, file = <file>)`. Touch points: `PartPathParser::findPartDirComponent` (`PartPathParser.cpp:150`, add beside the `kDetachedDirName` case at `:173`) and `route()` (`ContentAddressedMetadataStorage.cpp:636-664`).

Consequence: the clone publishes directly under the **final** ref `<part>`, and the subsequent `moveDirectory(moving/<part> → <part>)` (`DataPartStorageOnDiskBase::rename:692` → CA `moveDirectory:1024`) collapses to `src_key == dst_key` → the existing no-op (`ContentAddressedTransaction.cpp:1103-1104`). `moving/` staging stays load-bearing for swap crash-safety (`MergeTreeData.cpp:4106-4107`) — unchanged. (Rejected: cloning directly under the final dir — changes crash semantics; deferring publish to rename time — does not fit CA, where publish == commit.)
