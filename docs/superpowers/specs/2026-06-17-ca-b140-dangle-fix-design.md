...
> **⚠️ SUPERSEDED 2026-06-18 — the *mechanism* and the recommended fix below are INVALID. Do not implement from this spec. The PRODUCER is now PINNED (see next line); a new spec must be written for it.**
> **PINNED PRODUCER (ground-truth decode of the real delete-gen gc/snaps 1280/1281): shared/deduplicated-blob in-degree UNDER-COUNT.** GC's in-degree for a content-shared blob `B` counted only ONE parent (an older part's tree `T_live`). When that part merged away, `T_live` was retired and the cascade `stripTree(T_live)` dropped the only `T_live→B` edge GC knew of → `B` in-degree 0 → deleted — while a DIFFERENT live part references `B` via dedup, that edge never represented in the GC snapshot (243/243 VERDICT B; every healthy gen records `T→B` correctly via once-per-tree expansion, so it is NOT the marker/fold-edge mechanism this spec assumed). The local publish-gate handles same-process reuse (`CasReuseGcRace` passes); the CROSS-NODE dedup-vs-GC path is the producer. Open refinement (resolve first in the new spec): was the live part's dedup-publish BEFORE `B`'s delete (publish-gate/retire-view cross-node freshness) or AFTER (dedup-HEAD of an about-to-be-deleted blob)? — picks between (i) GC reachability-completeness for dedup-shared blobs vs (ii) cross-node dedup/publish-gate freshness.
