...
| Soak (a) churn — 30m full | **PASS** | S34: `fsck dangling=0`, `no unbounded leftovers=0`, `dropped content reclaimed to 0`, `event audit (no bad rows)=0`, `GC no Failed rounds=0`, `dryrun ⊆ deletable: 0/0`, 9/9. S35: `no bad CA-log events=0`, `create_errors=0`, `insert_errors=0`, final-table replica agreement equal, 14/14. `logs_archive/2026-08-03-stage-b-specimen/churn_s34_s35.log`, `SCENARIO_EXIT=0` | Maps onto the plan's (a) criterion: catalog entry count returns to baseline (`no unbounded leftovers`), zero alias reads (`event audit (no bad rows)`/`no bad CA-log events`), fsck clean (`fsck dangling=0`). Same wall-clock note as the smoke: ~1.5 min actual, not 30m, same fixed-cycle-count reason |
