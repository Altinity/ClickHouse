...
|---|---|---|---|---|
| `pending_deletes` | 108 (ch1 63, ch2 45) | `DiskS3DeleteObjects`=281,956; `DiskS3WriteRequestsCount`=281,956 (the delete-confirmation primitive rides a write-shaped request, not a `DELETE` verb) | total 209.3s; max single occurrence 16.0s (ch1) | `deleted`=`graduated`=`redeleted`=281,956; `spared`=32 (ch1 only — see T6b below), `absent`=`replaced`=0 |
| `manifest_deletes` | 108 (ch1 63, ch2 45) | `DiskS3DeleteObjects`=522,503; `DiskS3WriteRequestsCount`=522,503 | total 409.9s; **max single occurrence 184.3s (ch1)** | `attempted`=`deleted`=522,503 (0 declined); the 184.3s outlier correlates with the chaos-induced mount-fence trip during the `converge` stage, not steady-state cost — see Rounds-to-fixpoint row |
| `orphan_sweep` | 108 (ch1 63, ch2 45) | **UN-TIMED for S3-verb attribution** — `ProfileEvents` map is empty on every `orphan_sweep` row, both nodes (the phase's LIST/scan work evidently runs off-thread, same documented pattern as `meta_pool_wait`) | total 0.5ms (both nodes combined — the phase itself is near-instant; the untimed S3 work is elsewhere) | `deleted`=0, `listed`=`skipped`=14,586 (all four `retained_*` reasons read 0 — see criterion 1's explanation) |
| `ref_object_cleanup` | 108 (ch1 63, ch2 45) | `DiskS3DeleteObjects`=140,096; `DiskS3GetObject`=280,598; `DiskS3HeadObject`=420,694; read-side `S3ReadRequestsCount`=701,292, `S3ReadMicroseconds`=123.4s | total 268.2s; max single occurrence 56.0s (ch1, same chaos window) | `namespaces_planned`=216, `trim_enabled`=108 |
| `namespace_cleanup` | 171 phase-rows (ch1 69, ch2 102 — janitor runs more than once per round on some rounds) | `DiskS3GetObject`=446; `DiskS3HeadObject`=450; `S3ReadRequestsCount`=896; `S3ReadRequestsErrors`=4 (ch1, small, non-fatal — retried transparently per the AVAILABILITY line) | total 16.7s; max single occurrence 1.1s (ch1) | `janitor_pages`=171, `janitor_keys`=63,851, `janitor_deleted`=0, `leaked`=0 — same "nothing to reclaim in a clean run" pattern as `orphan_sweep` |
| Generation pruning (inside `round_commit`, no phase row of its own) | 108 (ch1 63, ch2 45) — **measurable after all, not un-timed**: `deletePrefixWholesale`'s own `DiskS3DeleteObjects`/`WriteRequestsCount` land directly on the `round_commit` row's `ProfileEvents`, cleanly separable | `DiskS3DeleteObjects`=270; `DiskS3WriteRequestsCount`=270 | total 0.53s; max single occurrence 23ms (ch1) | `generations_visited`=105, `generations_referenced`=213; `pruned_through` (a per-node high-water mark, not summable) reached 1830 (ch1) / 3735 (ch2) by run end |
| Rounds-to-fixpoint | Finish rows: ch1 `Success`=63 `Deferred`=6 `NotALeader`=80; ch2 `Success`=45 `Deferred`=57 `NotALeader`=477 (leadership churned hard after the mid-run mount-fence trip) | — | ch1 Success rounds: **68.8s/round average** (sum 4,332.4s / 63); ch2 Success rounds: **0.56s/round average** (sum 25.1s / 45) — a >100× disparity, entirely attributable to the isolated chaos-window stall (see below), not steady-state cost | Backlog first reaches all-zero and STAYS there from ch2 round 107 onward (criterion 3) |
