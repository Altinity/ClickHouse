...
| Soak (a) churn — 30m full | **RETRACTED, rerunning** | The initial `dev`-scale run's PASS verdict was RETRACTED: at `scale=dev` (the harness's default, which this run had not overridden) the plan's own criterion — "create/drop namespaces ≥1/s for ≥30 min under load" — was not actually exercised; the run finished in ~1.5 minutes. `scenarios.run --help` and `scenarios/cards/s34_s35_d1_churn.py`'s `param_table` confirm a real scaling knob exists (`--scale {dev,ci,full}`; `full` = 1000 S34 iterations / 600 S35 cycles vs `dev`'s 40/30). Re-running with `--scale full --seed 20260805 --duration 30m`; row filled once that completes | *(fill)* |
