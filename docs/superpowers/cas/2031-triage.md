...
**Исторический снимок (2026-08-23).** Этот файл — замороженный аудит указанных выше ревизий, а не
руководство по текущей реализации. Упоминания старых helper-методов, настроек и conditional blob
publication сохраняются как доказательства того среза. Текущий контракт задают
...
| CAS-040 | исправлено (`6333a986af6905e59f538855f61caf6dc296d1cf`, `f738450415013c0f3da8b72746eed7edd04ed16d`) | — | [план исправления](../plans/2026-08-21-cas-manifest-path-hygiene.md) | нет | Триаж воспроизвёл проблему обычным DDL: проекция с `\n` в имени оставляла недекодируемый осиротевший манифест и останавливала GC во всём пуле. Первый коммит ввёл единое экранированное представление пути в баннере, второй сделал sweep недекодируемого orphan-манифеста нефатальным для раунда. |
...
| подтверждено | 14 | реальная проблема на HEAD |
| not-a-bug | 8 | утверждение фактически неверно (галлюцинация) |
| исправлено | 4 | было реально, теперь закрыто |
| дубликат | 1 | та же земля, что у другого CAS-### |

Приоритеты: **P1 — 1**, P2 — 38, P3 — 83, без действия — 13.
...
Из четырёх исходных P1 CAS-001 закрыт коммитами `8e5ee61b6cc` и `11f5397a629`, CAS-058 —
коммитами `c5467b8989b` и `cfe9a6a3615`, а CAS-040 — коммитами
`6333a986af6905e59f538855f61caf6dc296d1cf` и `f738450415013c0f3da8b72746eed7edd04ed16d`.
Единственный оставшийся P1, CAS-106, пришёл из полевого отчёта.
...
## CAS-040 — Историческая запись: механика находки реальна и достижима обычным DDL (проекция с `\n` в имени), но следствие описано неверно: коммита нечитаемой части нет (INSERT падает fail-closed, данные не теряются) — зато осиротевший манифест навсегда заклинивает каждый раунд GC во всём пуле. (исправлено коммитами `6333a986af6905e59f538855f61caf6dc296d1cf` и `f738450415013c0f3da8b72746eed7edd04ed16d`; бывший P1) {#cas-040}

> **Закрыто 2026-08-22.** Ниже сохранён триаж состояния на момент находки. Коммит
> `6333a986af6905e59f538855f61caf6dc296d1cf` проводит путь entry через один escaper в строке
> записи и баннере, поэтому манифест faithfully carries arbitrary paths; коммит
> `f738450415013c0f3da8b72746eed7edd04ed16d` учитывает недекодируемый orphan-манифест и продолжает
> sweep. Поэтому upstream-изменение `ProjectionsDescription::getDirectoryName` не требуется.
...
## Что было внесено в BACKLOG (историческая запись)

На момент триажа была добавлена секция в
`docs/superpowers/cas/BACKLOG/formats-and-storage.md`. Тогдашнее направление фикса:
