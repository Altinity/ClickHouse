...
| Raw GC rebuild (`gc/state` disaster recovery) | **TODO (spec next)** | The "план Б" survivor of the 2026-06-30 raw-audit RFC: journals TRIM below sealed cursors, so lost/corrupt `gc/state` is NOT incrementally reconstructible — without a rebuild command the only safe fallback is "GC never runs again". One narrow command, derived-bookkeeping only, over-protect only (synthetic baseline + EMPTY retired lists), TLA+ `RawRebuildNoUnderProtect` gate. Registry-repair/orphan-sweep/debris-prune parts of that RFC are obsolete (registry removed by D1; sweeps live in regular rounds; no structural orphan-blob class per INV-2) |
