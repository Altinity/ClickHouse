...
| GC round is O(universe) not O(delta) — fold/discover skip-unchanged | **IN PROGRESS** (Phase 4 Lever A) | Spec `specs/2026-07-06-cas-gc-round-skip-unchanged-design.md`, plan `plans/2026-07-06-cas-gc-round-skip-unchanged.md`. A round making no destructive decision re-adopts the sealed in-degree generation (DEFER) instead of rebuilding it (~2×O(universe) snapshot read+write, ~1362 CasGcGet/idle round). Decision from cheap pre-fold signals (changed-shard count; graduation-due). Safety invariant: no destructive decision on a not-fully-folded snapshot ⇒ due graduation force-folds (mirror of the 2026-06-27 leak). Mandatory TLA+ gate `CaGcRoundDeferCore` (NoOverDelete + EventuallyFolded). Lever B (incremental point-updatable in-degree — makes even a non-idle small-delta round O(delta)) still open |
