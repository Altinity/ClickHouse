...
| GC round is O(universe) not O(delta) — fold/discover skip-unchanged | **TODO** (Phase 4, the O(N) apex) | Even with per-shard fold `Skip` (token-diff, `computeDiscoverDecisions`/`fold` already skip unchanged shards' READS), a round still does TWO full `cas/refs/` LISTs (`discoverUniverse` + `listRootShardTokens`) = O(namespaces×root_shards), re-reads the prior generation, and has no whole-round short-circuit — so 93 s @ 10k tables / 398 s @ 100k parts / ~2500 S3 ops per IDLE round. Fix: short-circuit a round to ~O(servers) when journal/ack-floor shows zero new transitions since the last sealed generation; prune untouched namespaces from the LIST sweep. Design-sensitive: spec + mandatory TLA+ gate (pruning must never skip an owner-removal event — the 2026-06-27 concurrent-leader leak failure mode). See `BACKLOG` S3-BUDGET idle-cost + O(ref universe) entries |
