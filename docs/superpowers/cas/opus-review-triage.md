...
78 пунктов обзора (T1-T12 blast radius + B1-B9 + M1-M12 + m1-m31 + NV-1…NV-14) перепроверены против
HEAD 2026-08-24.

| Статус на HEAD | Кол-во |
|---|---|
| подтверждено (всё ещё есть) | 33 |
| дубликат уже разобранного пункта | 18 |
| частично | 15 |
| не подтвердилось при перепроверке | 6 |
| исправлено | 4 |
| by-design / не разрешено | 2 |

Приоритеты остатка: **P1 — 3**, P2 — 25, P3 — 44, без действия — 5.

### Главное для релизной ветки {#release-headline}

**Из двух предмерджевых блокеров по не-CAS основаниям остался один.** T1 (диалект GCS, включавшийся
на весь клиент по `http_client = gcp_oauth`, с прослеженной регрессией Iceberg) закрыт коммитом
`faab6678d8f`. Остался **T2** — контракт кода возврата `clickhouse-disks`, причём худшей половиной:
сырой код ошибки возвращается как POSIX-статус и усекается до 8 бит, так что коды, кратные 256, дают
`exit 0` — провал, который скрипты читают как успех. Рядом с ним стоит **T4** — это не фикс, а
обязательство по доказательству: статическая изоляция disk-transaction contract держится, но нигде
не зафиксировано, что не-CAS полосы прогонялись с результатом «нет дельты против базы», а изменение
трогает каждую таблицу MergeTree.

### P1 — все три про один и тот же путь выключения {#p1-list}

B3, B4 и NV-9 — это одна цепочка, а не три пункта: `drainRefLanesForShutdown` ждёт `pending` и
`leader_active`, но не `pending_snapshot_publishes` (NV-9), поэтому detached-задача публикации
законно оказывается последним владельцем `Pool` (B4), и прощальная запись `~Pool` эмитит событие
через `Context`, у которого `shared` уже обнулён (B3) — разыменование нуля на фоновом потоке при
выключении. Нужный цикл ожидания уже существует в том же файле дважды, так что фикс — пересадка с
бюджетом, а не новая машинерия. В `final-checks-todo.md` пунктом 10.

### Что перепроверка добавила сверх обзора {#new-findings}

- **B8** — единственный по-настоящему новый блокер и одновременно пробел в моих же предыдущих
  триажах: `SYSTEM CAS GC REBUILD … FORCE` правильно классифицирует недекодируемый `gc/state` как
  свою катастрофу, проходит гейт — и падает `CORRUPTED_DATA` на повторном декоде тех же байт внутри
  взятия lease. CAS-069 и CAS-095 останавливались раньше по цепочке; тестов нет, потому что все
  rebuild-тесты состояние **удаляют**.
- **NV-5** — вопрос обзора был задан не про тот дискриминатор: `eraseView` аллоцирует своей первой
  строкой (`String cache_key = key.cacheKey()`), безусловно, а вызывается снаружи `try` из двух
...
Шесть пунктов не подтвердились, и три ошибки стоит знать поимённо: **T6** инвертирован (в апстриме
был `chassert` и следом безусловное разыменование, то есть segfault в release — новый throw строго
лучше); **T9** — таблицы не материализуются на старте, `prepareTable` срабатывает только на непустом
флаше; **M7** — «под дефолтами каждый доступ платит HEAD» неверно, тёплый `CachedForLoad`-хит
обслуживается без обращений к бэкенду. Ещё два уточнения: у m16 предложенный фикс невозможен (LIST —
вход самого вердикта defer), а m17 попадает под стоящее вето на изменение протокольного шага.
