...
| CAS unit-test gate | run the gtest binary with `--gtest_filter='CAS*'` — every CAS suite carries the `CAS` prefix by convention; a suite that escapes the filter escapes the gate (this happened twice; check new suites) |
