...
**It is PRE-EXISTING, and this is settled by BEHAVIOUR rather than by code age alone.** A grep of the
whole 36 GB log archive finds the class 62 times in the pre-Task-14 archives, every hit dated
`2026.07.26` — three days before the Task 15/16 merge — across **33 distinct parts** in the
`default.s42_alloc` scenario, alongside 67,957 `mount lease not held` lines. The identical condition
was present then at far greater volume, and this re-validation's instance is markedly milder: one part
against thirty-three. `git blame` agrees from the other side, dating the CA throw site to
`21d207734095`, 2026-07-23. Soak 1's absence of the class could never have settled this, since that run
died at 49 minutes without reaching a blip.
...
The induced-blip reproducer (RCA appendix, `25ca4cffc7d`) then SHARPENED that shape twice over. First,
the path is ref-plane-NEUTRAL: `system.content_addressed_log`, captured complete this time, shows zero
CA events at the detach announcement — no ref drop, nothing dangling, no re-publish owed, so the
verdict's re-open condition was checked and not triggered. Second, under this timing the detach never
actually executes: the move would have to touch the very disk whose not-Live state triggered the
remediation, so the announced "Removing it" removes nothing — the part stayed ACTIVE with equal rows on
both replicas and `part_log` carries no remediation row. What survives as risk is the interleaving
where the disk recovers between decision and move — the double-blip shape — and that is exactly what
the chartered fix removes by never reaching the decision. Robustness fix now, not a data-safety one.
