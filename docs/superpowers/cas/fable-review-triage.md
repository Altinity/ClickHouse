...
| ID | Статус | Приоритет | До релиза? | Где отслеживается | Суть |
|----|--------|-----------|------------|-------------------|------|
| V1 | подтверждено | P3 | нет | не отслеживается (смежное — `docs/superpowers/cas/2031-triage.md` CAS-011 `{#cas-011}`,… | Исчерпывающий обход всех durable/conditional write-сайтов CAS-дерева проведён — из ~40 сайтов ровно ОДИН пишущий сайт writer-плоскости остался без `checkFenceOrThrow`: `createNamespaceStep1` (шаг 1 рождения namespace в `cas/ref_catalog`). |
| V2 | не подтвердилось | — | — | — | Три непрочитанных тела `CasRefLedger` (wedge-resolution, recovery-walk, snapshot-publish) прочитаны целиком — дефектов нет, покрытие тестами есть по всем трём (тоньше всего у snapshot-publish); объём при этом сильно больше заявленных ~1300 строк. |
| V3 | не подтвердилось | — | — | — | `check_objects_after_upload` вообще не сравнивает ETag (только существование и размер), поэтому к подстановке generation-номера GCS нечувствителен; реальная же экспозиция `gcp_oauth` — сплошная подмена ETag на всём трафике — закрыта коммитом `faab6678d8f`. |
| V4 | частично | P2 | нет | не отслеживается; смежное — `docs/superpowers/cas/2031-triage.md` CAS-063 `{#cas-063}` … | «Accepted residual window» в `CasDecommission` — деструктивная фаза от неё НЕ страдает (её всё время прикрывает mount-лиз жертвы), но окно реально и даёт другой дефект: живому одноимённому преемнику ошибочно ставится `retired_at_ms`, и он упадёт при СЛЕДУЮЩЕМ рестарте. |
| V5 | не подтвердилось | — | — | — | Коллизия same-txn в `moveDirectory` достижима ТОЛЬКО внутренним багом сборки op-списка самой транзакции ⇒ `LOGICAL_ERROR` классифицирован верно, менять на `CORRUPTED_DATA` не нужно. |
| V6 | не подтвердилось | — | — | — | Обе половины пункта закрыты чтением: `copyObjectConditional` схлопывает в `{created=false}` ТОЛЬКО настоящий 412, а промоут-вызывающий всё равно пере-наблюдает HEAD'ом; `ReadBufferFromFileView::prefetch()` действительно correct-by-omission. |
| V7 | частично | P3 | нет | — | `4fcc40f97d9` действительно описывает намного больше своего 6-строчного диффа, но оба названных коммита живут НЕ в `cas-gc-rebuild`, а только в `feature/antalya-26.6/CAS`; `1c5ecf22511` как расхождение message/diff не подтверждается. |

## Итоги перепроверки {#verdict-summary}

42 пункта обзора (4 блокера + 9 major + 11 minor + 11 nits + 7 «needs verification») перепроверены
против HEAD 2026-08-22.

| Статус на HEAD | Кол-во |
|---|---|
| подтверждено (всё ещё есть) | 28 |
| частично (закрыта часть) | 7 |
| не подтвердилось при перепроверке | 4 |
| дубликат пункта триажа #2031 | 2 |
| исправлено | 1 |

Приоритеты остатка: **P1 — 3**, P2 — 8, P3 — 27, без действия — 4.

**Главный вывод: за 16 дней из 42 пунктов закрылся ровно один** — M9 (GCS `gcp_oauth` переписывал
все запросы клиента), и то работой параллельной сессии (`faab6678d8f`). Ни rev.8 disk-lifecycle, ни
writepath stage-1, ни TXN-ONE-PIPELINE ни одного пункта этого обзора не закрыли: они шли по другим
направлениям. Разделы minor и nits не тронуты вообще — 22 пункта из 22 воспроизводятся дословно,
и единственная разница с обзором — сместившиеся номера строк после переезда файлов (`592b9b83568`).

### Три P1 — в предрелизном списке {#p1-list}

Все три были **не отслежены нигде** до этой перепроверки; занесены пунктом 9 в `final-checks-todo.md`.

| ID | Суть | Почему P1 |
|---|---|---|
| B1 | `~Gc` отсутствует, `meta_pool` объявлен до `condemn_marker_mutex` → живой воркер лочит разрушенный мьютекс | UB класса heap corruption; rev.8 сделал разрушение `Gc` штатным (UNMOUNT, GC STOP), то есть окно расширилось |
| B2a | `MountLeaseKeeper::claim()` бросает `LOGICAL_ERROR` на четырёх ветках «окружение изменилось» | в debug/ASan это `abort()` на фоновом потоке — ровно на полосах, которые сертифицируют фичу; соседний renewal-путь давно переписан на `ABORTED` |
| B3 | inline `disk(metadata_type='cas')` минтит члена пула в обход всех семи привилегий `SYSTEM CAS` | межтенантная эскалация: любой, кто может `CREATE TABLE`, получает пул-широкий обзор чужих namespace |

### Что перепроверка добавила сверх обзора {#new-findings}

- **Обход fence-сайтов доведён до конца** (обзор оставил его открытым как V1): из всех durable- и
  условных записей CAS ровно один writer-plane сайт без проверки fence —
  `createNamespaceStep1` (`Pool/CasRefCatalog.cpp:200-230`), единственный вызывающий `casUpdateImpl`
  в обход обязательства, задокументированного в его же заголовке. Самоизлечивается (шаг 3 откажет),
  поэтому P3, но это durable-мусор в пул-глобальном каталоге и на него нет теста.
- **V4 переформулирован**: заявленная опасность («sweep удалит нужное преемнику») не подтвердилась —
  деструктивные фазы идут под собственным mount-lease жертвы. Зато нашёлся другой дефект того же
  окна: cross-key TOCTOU между liveness-перепроверкой и CAS owner-анкера позволяет проштамповать
  `retired_at_ms` ЖИВОМУ преемнику с тем же UUID, и его следующий рестарт будет отбит как
  `CORRUPTED_DATA`.
- **Четыре пункта самого обзора оказались неверны**: B4 («B66a нигде не отслеживается» — артефакт
  ветки поставки, где нет `docs/superpowers/`; описанный механизм устарел после B181), V2, V3, V5,
  V6 — снятые при доведении проверки до конца.
- **Уточнения к цифрам обзора**: call-site'ов retry-later 63, а не «40+»; неверный префикс `ca-` в
  документации встречается в 9 местах 6 файлов, а не на одной странице; «~10 мест, где правило
  „одна часть = одна транзакция“ переписано заново» — это состояние уже ПОСЛЕ частичной
  централизации, которая была и в дереве обзора.

### Что делать с остатком {#what-next}
...
## Needs verification — детали {#needs-details}

### V1 (подтверждено, P3) {#v1}

Метод обхода. Единственная durable-поверхность CAS — мутирующие методы `Cas::Backend` (`Backend/CasBackend.h:230-382`): `putIfAbsent`, `putIfAbsentStream`, `putOverwrite`, `casPut`, `deleteExact`, `promoteStaged`, `resurrect`. Плюс один не-backend путь — `objectStorage()->writeObject` для S3-staging (`ContentAddressedTransaction.cpp:879`). Перечислены ВСЕ их вызовы вне тестов/бенчмарков/самих реализаций бэкендов.

Плоскости и их гейты (важно для классификации):
- writer-плоскость (mount lease) — гейт `CasMountRuntime::checkFenceOrThrow(admitted_generation)` (`Pool/CasMountRuntime.cpp:97-113`) либо предикатная форма `refAppendFenceOk`/`fence_ok` (`:115-127`);
- GC-плоскость — свой лидерский лиз `gc/state` (token-CAS) + exact-token deletes;
- административная плоскость (decommission) — свой claim; в коде явно: «no mount-lease fence applies to it — the exact-token CAS itself is the safety mechanism» (`Tools/CasDecommission.cpp:412-416`);
- bootstrap (до появления mount-инкарнации) — `Pool::open` / probe.

A. Сайты writer-плоскости — ЗАФЕНСЕНЫ:
1. `Pool/CasPlainObjects.cpp:46` `putIfAbsent` и `:51` `putOverwrite` — `admitted_generation` на `:38`, `check_fence_or_throw_fn` на `:44` внутри цикла ретраев. ✓
2. `Pool/CasPlainObjects.cpp:83` `deleteExact` — `:75` + `:82`. ✓
3. `Pool/CasRefCkpt.cpp:288` `casPut` (`_ckpt`) — `check_fence_or_throw(admitted_generation)` на `:267` «AFTER the read, BEFORE the CAS, on EVERY attempt», плюс ещё две проверки на `:234` и `:256`. ✓
4. `Pool/CasRefCatalog.cpp:414` `casPut` (`deleteCompletedRemovingAtSnapshot`) — `check_fence` на `:404` и `:429`. ✓
5. `Pool/CasRefCatalog.cpp:127` `casPut` (`casUpdateImpl`) — сам цикл фенс НЕ проверяет (это задокументировано как обязанность вызывающего: `Pool/CasRefCatalog.h:84-95`), и все его вызывающие, КРОМЕ одного, обязанность выполняют: `beginRemoving` (`:301`), `cancelStalledCreating` (`:470`), `completeCreation` (`:521`), `reconcileStaleCreator` (`:621`) — все вставляют `check_fence_or_throw(admitted_generation)` первым действием в `mutate`. ✓
6. `Pool/CasBlobMeta.cpp:29,36` (`putMetaIfAbsent`/`casMeta`) → `Pool::stagingPutIfAbsentMutable`/`stagingConditionalOverwrite` (`Pool/CasPool.cpp:1757,1752`) → `CasRefLedger.cpp:265,260` → контроллер с `fence_ok_fn`. ✓
7. `Pool/CasPartWriteTxn.cpp:605` `promoteStaged` и `:611` `putIfAbsentStream` — обёрнуты в `one_attempt`, который отдаётся в `store->stagingConditionalCreate` (`:623`) → `CasRefLedger.cpp:253` → `conditionalCreateControlled(..., fence_ok_fn)`. ✓
8. `Pool/CasPartWriteTxn.cpp:732` и `:759` `resurrect` — сырые вызовы бэкенда, но с явным `store->checkFenceOrThrow(displace_admitted_generation)` непосредственно перед (`:724` и `:754`; захват генерации на `:700`). ✓ (закрыто коммитом `1fe585ea078`, см. CAS-088)
9. `Pool/CasPartWriteTxn.cpp:880` `stagingPutIfAbsent` (манифест) → `CasRefLedger.cpp:246` → контроллер с `fence_ok_fn`. ✓
10. `ContentAddressedTransaction.cpp:879` `writeObject` (S3-staging) — генерация захвачена на `:890`, колбэк `check_fence_before_finalize` на `:904` вызывается прямо перед `sink->finalize()` (`ContentAddressedTransaction.h:339-391`). ✓
11. Весь ref-лейн `CasRefLedger`: ВСЕ вызовы контроллера передают реальный предикат — `fence_ok_fn` (`:246,253,260,265`), `admitted_fence_ok` (`:1072-1082` snapshot-seal, `:2207-2220` wedge `slotOccupy`), `fence_ok` (`:3187,3559` append), `runtime_still_admitted` (`:4235-4241,4327`). Все они = `fence_ok_fn() && !cancel && fence_generation_fn() == admitted`. ✓
12. Сам контроллер (`Backend/CasRequestControl.cpp`) проверяет `fence_ok()` перед КАЖДОЙ попыткой и после успеха: `putIfAbsentControlled` `:336,410`; `conditionalCreateControlled` `:432,470`; `putOverwriteControlled` `:513,536,564`; `putIfAbsentControlledMutable` `:595,617,645`; `slotOccupy` `:682` (только pre-attempt, осознанно задокументировано на `CasRequestControl.h:562-575`); backoff-пауза тоже фенсится (`:251`). ✓

B. Сайты ВНЕ writer-плоскости (фенс неприменим по конструкции, свой гейт назван):
13. `Pool/CasServerRoot.cpp:151` (owner-anchor), `:272` (writer-epoch CAS), `:363/:398/:430` (mount-lease claim/renew), `:941/:996/:1186/:1337` (`SingleWriterSlot`/`MountLeaseKeeper` claim/renew) — это САМ протокол, который фенс минтит; защита — token-exact `putOverwrite(..., last_token)`.
14. `Pool/CasServerRoot.cpp:663` — GC fences-out просроченный mount (GC-плоскость).
15. `Pool/CasPoolMeta.cpp:92,155` — bootstrap пула в `Pool::open` (`Pool/CasPool.cpp:493` рядом), mount-инкарнации ещё нет; защита — token-CAS + `allow_mint`-гейт.
16. `Backend/CasProbe.cpp:38,58,73,101,115,133,142,151,166,180,213,246` — capability-батарея по ключам `_probe/*` на этапе mount-open, до лиза.
17. GC-плоскость: `Gc/CasGc.cpp:802` (blob delete, за `suppress_destructive`-гейтом `:797-800`), `:973`, `:1076`/`:4377` (round-commit `gc/state` CAS), `:1193`, `:1243`, `:3461`, `:3565/:3569`, `:4504` (heartbeat), `:4528/:4549/:4603` (acquire/renew/steal лиза); `Gc/CasGcMaintenanceState.cpp:34`; `Gc/CasBlobInDegree.cpp:343` (write-once детерминированный артефакт с byte-equal adopt); `Gc/CasOrphanManifestSweep.cpp:586` (premise-gated exact-token). Гейт — лидерство `gc/state` + token-exactness.
18. `Gc/CasNamespaceJanitor.cpp:111` `deleteExact` — ЕСТЬ явный `if (!fence_held()) break;` прямо перед (`:104-108`, повтор на `:125`). ✓
19. `Tools/CasDecommission.cpp:68`, `:92`, `:422` — административная плоскость; `:422` идёт даже через контроллер, но с `[]{ return true; }` — осознанно, обоснование в комментарии `:409-419`.
20. `ContentAddressedTransaction.cpp:197` `removeObjectIfExists` — удаление СВОЕГО staging-объекта со случайным именем на abort-пути; эксклюзивность неприменима.

C. НАЙДЕННЫЙ ПРОБЕЛ (ответ на вопрос ревью — «да, один пропущенный сайт есть»):
`createNamespaceStep1` (`Pool/CasRefCatalog.cpp:200-230`) — единственный вызывающий `casUpdateImpl`, который НЕ выполняет «the fence obligation» из `Pool/CasRefCatalog.h:84-95`: его `mutate` (`:215-224`) проверяет только присутствие строки, `check_fence_or_throw` в нём нет, и сама функция даже не принимает ни `admitted_generation`, ни колбэк. При этом её вызывающий `CasRefCatalog::createNamespace` (`:543-596`) держит оба параметра в области видимости и передаёт их в шаг 3 (`completeCreation`, `:596`) — то есть асимметрия внутри одной функции.
Окно: `CasRefLedger::resolveNamespaceLife` (`Pool/CasRefLedger.cpp:1213-1240`) захватывает `admitted_generation` выше по стеку и до `createNamespace` (`:1239`) фенс не перепроверяет; между захватом и CAS шага 1 умещаются полный `CasRefCatalog::read` (`:1228` и `:557`) и ещё один read внутри `casUpdateImpl` (`:120`) — это несколько сетевых RTT.
Последствие: mount, потерявший лиз (или пере-armed под свежей инкарнацией), может durable вставить строку `Creating` со свежесминченной инкарнацией namespace (`:578-579` `mintFreshIncarnation`) под УЖЕ МЁРТВЫМ `CreatorFence{server_root_id, live_epoch, admitted_generation}` (`Pool/CasRefLedger.cpp:1224`). Это ровно тот класс, который правило rev.7 [C2] запрещает: durable-эффект от писателя, потерявшего эксклюзивность.
Почему P3, а не выше: (а) следом идущий `completeCreation` немедленно упрётся в `check_fence_or_throw` (`:521`) и вернёт `FencedOut`, так что до `Live` строка не дойдёт; (б) вставленный фенс терминален по построению, поэтому следующий открыватель заберёт строку через `reconcileStaleCreator` (`Pool/CasRefLedger.cpp:1283`) или отменит её через `cancelStalledCreating` (`:4887`); (в) потери данных нет. Остаток — durable-мусор в пул-глобальном `cas/ref_catalog` (объект, который перечитывает КАЖДАЯ публикация парта, см. CAS-112) плюс лишняя мёртвая инкарнация namespace для janitor'а. Худший случай — писатель, которого FORGET-декоммишенули: строка ссылается на `server_root_id`, которого больше нет.
Фикс тривиален и симметричен остальным четырём вызывающим: протащить `admitted_generation`/`check_fence_or_throw` в `createNamespaceStep1` и вызвать проверку первой строкой его `mutate`.
Тестовый пробел: `src/Disks/tests/gtest_cas_fence_generation.cpp` покрывает plain-object PUT/DELETE (`:220,240,267`), S3-staging finalize (`:291`), condemned `putOverwrite`/`resurrect` (`:382,430`) — каталожной ветки нет ни одного теста.
Вторичное наблюдение того же класса (ниже уверенность, не считаю отдельным сайтом): `Pool/CasPartWriteTxn.cpp:1433` `deleteExact` на abort-пути удаляет staged-манифесты этой транзакции без проверки фенса и внутри `catch(...)`-глушилки (`:1436-1439`); удаление token-exact по свежему HEAD, но от захвата токена не защищает от случая, когда манифест с ТЕМ ЖЕ content-ref уже адоптирован живой транзакцией другого писателя. Abort-путь особенно достижим именно при потере лиза.

Вывод по V1: заявленное ревью подозрение подтверждается — один пропущенный сайт writer-плоскости существует (`createNamespaceStep1`); все остальные ~19 групп сайтов либо зафенсены напрямую/через вызывающего, либо принадлежат другой плоскости с явно названным собственным гейтом.

### V2 (не подтвердилось, —) {#v2}

Реальный объём: `Pool/CasRefLedger.h` — 1247 строк, `Pool/CasRefLedger.cpp` — 5124, итого 6371 (оценка ревью «~1300 строк» относилась, видимо, только к трём телам). Сами тела: `resolveWedgeOnce` — `Pool/CasRefLedger.cpp:2125-2613` (489 строк); `runRecoveryWalkOnce` — `:738-1211` (474 строки); snapshot-publish размазан по семи функциям — `hasStateBearingSnapshotCandidateUnderStateLock` (`:3948`), `admitSnapshotPublishUnderStateLock` (`:3957`), `dispatchSnapshotPublisher` (`:3994`), `settleSnapshotPublish` (`:4035`), `maybeScheduleSnapshotPublish` (`:4062`), `advancePublishBackoff`/`resetPublishBackoff` (`:4082-4099`), `tryPublishSnapshotAndAdvanceCheckpointOnce(OnRuntime)` (`:4219-4435`), ~364 строки.
Проверялось прицельно то, что ревью назвало: глотание ошибок, неточные удаления, застревание wedge, неограниченные циклы, лок через I/O.
— Локи: все `std::lock_guard` покрывают только чтение/запись состояния; `slotOccupy`, `publishCkptContribution`, `backend.get` вызываются вне лока. Лока через I/O не найдено.
— Глотание: единственное — `rt->state.materializeCommitted()` в `:2493-2504`, и оно осознанное и объяснено на месте («SWALLOWING, symmetrically with the ordinary commit arm… a mid-fold allocation failure merely defers the fold… must not unwind past a completed install»).
— Границы: straggler-seal цикл ограничен `kRefRecoveryMaxSlotAttemptsPerEpoch` (`:1050-1056`); внешний `for(;;)` walk (`:888`) ограничен конечным committed/durable логом и на каждой итерации перепроверяет допуск через `checkRecoveryStillAdmitted`.
— Wedge: при неоднозначном исходе I/O wedge НЕ снимается (`StillWedged`/`Reason::ResolveFoundNothing`), что соответствует инварианту acked-then-lost.
Покрытие: wedge — `src/Disks/tests/gtest_cas_ref_wedge_every_attempt.cpp` (25 TEST, 1476 строк, 28 прямых обращений к `resolveWedgeOnce`/`forceWedgeForTest`/`refLaneWedgedForTest`), смежно `gtest_cas_ref_lane_exception_safety.cpp` (3) и `gtest_cas_ref_statemachine.cpp` (67); recovery — `gtest_cas_ref_recovery_cas_walk.cpp` (39 TEST, 2010 строк); snapshot-publish — `gtest_cas_ref_snapshot_publish_ordering.cpp` (4 TEST, 501 строка) плюс общий примитив в `gtest_cas_ref_ckpt.cpp` (37 TEST). Тоньше всего snapshot-publish (4 теста) — это наблюдение, а не дефект.

### V3 (не подтвердилось, —) {#v3}

`gcp_oauth` — значение селектора `http_client` (`S3ObjectStorage`/`S3::ClientConfiguration`), строго opt-in: пользователь должен явно выставить `http_client = gcp_oauth` (или `gcs_hmac`). Оно и включает `Client::supportsGcsNativeConditionalRequests()` (`src/IO/S3/Client.cpp:990`).
ETag-чувствительность: `check_objects_after_upload` в `src/IO/WriteBufferFromS3.cpp:228-236` и `checkObjectAfterUpload` в `src/IO/S3/copyS3File.cpp:294-299` вызывают только `S3::checkObjectExists` (HEAD) и, в пути `WriteBufferFromS3`, сравнивают `getObjectSize()` с ожидаемым числом байт. `ETag` не читается и не сравнивается нигде ⇒ подстановка generation-номера ни ложного прохода, ни ложного отказа здесь дать не может. Это верно одинаково для S3, GCS-via-HMAC и GCS-via-OAuth.
Реальная (отдельная) экспозиция: до `faab6678d8f` выбор `gcp_oauth`/`gcs_hmac` применял GCS-диалект — срезание AWS-заголовков, переименование `x-amz-*`→`x-goog-*` и перезапись ответного `ETag` значением `x-goog-generation` — ко ВСЕМ запросам этого клиента, а не только к условным записям CAS. Пострадал бы любой не-CAS условный писатель на том же диске (например Iceberg-обновление `version-hint.text` с настоящим hex-`ETag` в `If-Match`), то есть все пользователи этой настройки `http_client`, а не только пользователи CAS. Коммит `faab6678d8f` («Isolate GCS generation adaptation to explicitly marked CAS requests», 2026-08-21) сужает диалект и подстановку ETag до запросов, которые CAS явно помечает `NativeConditional`, возвращая остальному трафику upstream-семантику `ETag`. На `cas-gc-rebuild` вопрос закрыт.

### V4 (частично, P2) {#v4}

Комментарий — `Tools/CasDecommission.cpp:362-370`, код хвоста — `:371-446`.
(1) Преемник с тем же UUID возможен и это НОРМА, а не экзотика: UUID не ротируется при рестарте, `claimOwnerOrThrow` (`Pool/CasServerRoot.cpp:113-177`) при `owner->server_uuid == our_uuid` (`:125`) просто принимает идентичность. Сам инструмент декоммишена тоже монтируется ПОД UUID жертвы (`Pool/CasPool.cpp:807-836`).
(2) Деструктивная часть закрыта: `openForDecommission` держит и продлевает mount-лиз жертвы всю фазу drop namespace (`:177-211`), sweep манифестов (`:213-230`), drain staging (`:232-236`) и mountpoint (`:238-242`). Настоящий преемник в это время получает `LiveDoubleStart`. Лиз отпускается только на `admin.reset()` (`:309`) — уже ПОСЛЕ всех удалений. То есть формулировка ревью «удалит объекты, нужные преемнику» на HEAD не материализуется.
(3) Реальное окно — узкий хвост retirement'а ПОСЛЕ `:309`: liveness-recheck перечитывает `mount_key`/`epoch_key` (`:380`, `:390`) и отменяет томбстоун, если они вернулись (`:399-403`); но обычный рестарт преемника, приземлившийся между этими GET'ами и GET+CAS owner-анкера (`:408`, `:421-423`), невидим.
(4) Почему token-exactness не спасает: томбстоун — exact-token CAS по `owner_key`, а нормальный reclaim `owner_key` вообще не переписывает (`Pool/CasServerRoot.cpp:123-129` только читает). Преемник меняет `mount`/`epoch`, токен `owner_key` не трогает ⇒ CAS проходит. Это cross-key TOCTOU: exact-token защищает от гонки на ТОМ ЖЕ ключе, но не от изменения ДРУГОГО ключа, проверенного раньше.
(5) Последствие: `throwIfOwnerRetired` (`Pool/CasServerRoot.cpp:74-85`) вызывается только на mount-claim (`:123-129`, `:165-171`), поэтому живой преемник в текущем запуске не пострадает — но его следующий рестарт будет отвергнут как `CORRUPTED_DATA` «was explicitly decommissioned» и потребует ручной очистки поля (см. CAS-011-смежное описание томбстоуна в 2031-triage `:1194`). Это доступностный false-refusal, не потеря данных.
Остаток «не разрешено» только эмпирический: попадание рестарта ровно в двухGET'овую щель — тайминг, статикой не подтверждается; нужен chaos-тест, гоняющий рестарт в этот зазор.

### V5 (не подтвердилось, —) {#v5}

Тест — `src/Disks/tests/gtest_ca_wiring.cpp:1219-1248` (+ death-test `:1254-1262`). Throw — `ContentAddressedTransaction.cpp:1349-1352` внутри `moveDirectory` (начало `:1226`).
Обе сравниваемые стороны берутся из ОДНОГО контейнера — члена самой транзакции: `if (auto src_it = parts.find(src_key); src_it != parts.end()) { ... PartStaging & dst_st = parts[dst_key]; PartStaging & src_st = src_it->second; }` (`:1331-1335`), где `parts` — `std::map<std::pair<std::string,std::string>, PartStaging>` (`ContentAddressedTransaction.h:157`), in-memory однопоточный staging-оверлей, заполняемый исключительно операциями ЭТОЙ транзакции до коммита. В теле сравнения (`:1346-1348`) нет ни чтения durable-каталога, ни состояния другой транзакции, ни существующего каталога на диске.
Следовательно ни второй писатель, ни конкурентная транзакция, ни пользовательский ввод не могут наполнить ни одну из сторон. Собственный комментарий кода говорит то же (`:1338-1345`): «a genuine collision is not expected in normal operation — only some future op-order re-keying the same file under both stagings», то есть гипотетический баг НАШЕГО порядка операций.
Правило `feedback_logical_error_tests_death_split` соблюдено: путь не input-reachable, переклассификация в `CORRUPTED_DATA` была бы ошибкой.

### V6 (не подтвердилось, —) {#v6}

(a) `copyObjectConditional` — `src/Disks/DiskObjectStorage/ObjectStorages/S3/S3ObjectStorage.cpp:902-914`: `if (exc.isPreconditionFailed()) return {.created = false, .dest_etag = {}};` (`:906-907`), а `throw;` (`:909-911`) для ВСЕГО остального — сеть, таймаут, access-denied, 5xx, неклассифицируемое. То есть неопределённый исход в «peer won» не схлопывается вообще: 412 — это определённый ответ самого S3, вычислившего precondition.
Вызывающий: `ObjectStorageBackend::promoteStaged` (`CasObjectStorageBackend.cpp:1097-1132`, вызов на `:1117-1118`) на `!res.created` возвращает `PutOutcome::PreconditionFailed` (`:1126-1127`), исключения перебрасывает (`:1120-1124`). Дальше `CasRequestController::conditionalCreateControlled` (`Backend/CasRequestControl.cpp:423-501`): возврат `PreconditionFailed` → `CasCreateOutcome::Occupied` (`:465-466`), а ИСКЛЮЧЕНИЕ идёт через `classifyConditionalWriteResult` (`:43-62`), где сеть/таймаут/неизвестное → `Unresolved` (`:57-61`) и только там выполняется HEAD-резолв занятости (`:478-495`). То есть различение `Unresolved` vs definite loss не стёрто — оно живёт уровнем выше и ключуется на «бросил vs вернул».
Хард-фейла нет: `CasPartWriteTxn.cpp:628-629` возвращает `PreconditionFailed`, и вызывающий пере-наблюдает сам — `const HeadResult hr = store->backend().head(key);` (`CasPartWriteTxn.cpp:661`), затем проверки существования (`:662-676`) и condemned-vs-live (`:678-687`) перед adopt/resurrect. Именно `:661` и закрывает вопрос, при опоре на content-addressing (сценарий «SDK-ретрай CopyObject увидел собственный 412» разобран в комментарии `S3ObjectStorage.cpp:810-830`).
(b) `ReadBufferFromFileView::prefetch()` (`src/IO/ReadBufferFromFileView.cpp:33-36`) — `executeWithOriginalBuffer([&]{ impl->prefetch(priority); });`, без ребейза. Остальные методы ребейзят `file_offset_of_buffer_end` из `impl->getPosition() + impl->available()` сразу после вызова: `nextImpl` (`:89,97`), `seek` (`:121,131`), `setReadUntilPosition` (`:53,55`), `setReadUntilEnd` (`:67,69`) — дисциплина, введённая фиксом B115 `440871098a9`.
Ребейзить в `prefetch` нечего: контракт `ReadBuffer::prefetch` (`src/IO/ReadBuffer.h:188`) — advisory, без возврата и без мутации буфера; `AsynchronousBoundedReadBuffer::prefetch` (`src/Disks/IO/AsynchronousBoundedReadBuffer.cpp:156-173`) только ресайзит `prefetch_buffer` и кладёт `std::future` в `prefetch_future`, не трогая `working_buffer`/`pos`/`file_offset_of_buffer_end` — подмена буфера и сдвиг офсета происходят позже, в `nextImpl`; `AsynchronousReadBufferFromFileDescriptor::prefetch` (`src/IO/AsynchronousReadBufferFromFileDescriptor.h:24-35,66`) устроен так же. Оба конкретных impl'а, которые вью может обернуть (S3-путь через async-reader и локальный файл из `PackedFilesReader.cpp:102`), синхронно позицию не меняют. Неверных байтов не возникает.

### V7 (частично, P3) {#v7}

`4fcc40f97d9` («LocalObjectStorage snapshot listing hardening») — подтверждено. Сообщение обещает «a file vanishing between listing and stat is skipped, not a filesystem_error; non-recursive explicit-stack walk with error_code overloads and a symlink guard; fail closed on an embedded-NUL path». Фактический дифф — 6 строк: добавлена проверка `fs::is_directory` в `tryGetObjectMetadata`, чтобы каталог трактовался как отсутствующий объект (обход B38 для `remote_data_paths`). Всё остальное описанное уже существовало в базе и было внесено более ранними коммитами той же линии: `4bce1b998ad`, `3dfcce74d6d`, `7a3282dee7f`, `82899b24723`, `88a9f334f1a`, `dbf333d46c3`, `e8816bb304a`.
`1c5ecf22511` — НЕ подтверждено как расхождение сообщения и диффа: сообщение («CAS subsystem: microbenchmarks») точно соответствует диффу (+557/-0: новый `benchmarks/benchmark_cas_ref_protocol.cpp` на 553 строки плюс CMake-обвязка). Что действительно есть — шапка добавленного файла цитирует результаты бенчмарков, датированные 2026-07-19/07-20/07-21, то есть пересказывает историю работы, приземлённой другими коммитами. Это содержимое комментария внутри добавленного файла, а не завышение вклада коммита. Других «per-layer squashed commits» ревью поимённо не называет, проверить их нечего.
Существенная поправка к самой постановке вопроса (меняет применимость): оба коммита НЕ являются предками текущего HEAD `cas-gc-rebuild` — они есть только на `feature/antalya-26.6/CAS` (база ревью `056488b47a0` — там же; проверено `git merge-base --is-ancestor`). `cas-gc-rebuild` разошёлся около 2026-08-04 и несёт собственные, корректно нарезанные коммиты на ту же функциональность (`6c3a678550b` «CAS M6: B38 make remote_data_paths traversal directory-safe on CA disk», `82899b24723` «List local object storage with an explicit directory stack») — то есть именно ту гранулярную историю, которой ревью и не хватало. Признаков reword/amend нет (оба находятся по точному хэшу в `git log --all`, что reword бы сломал).
Действие: неприменимо. Переписывать историю запрещено политикой проекта, а на релизной ветке проблема снялась сама собой за счёт независимой нарезки коммитов.
