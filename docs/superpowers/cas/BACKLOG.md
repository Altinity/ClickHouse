...
- **[ckpt-token-cache] `_ckpt` GET elision on the committed-append path** — DESIRABLE (write-path; noted 2026-08-03, user-directed) — {#ckpt-token-cache} — Since INV-4 landed (`eeadbc1887e`, Stage A), every committed ref-log chunk pays `GET _ckpt` + token-CAS *serially after* the log `PUT` (spec `2026-07-27-cas-ref-chain-complete-cut-design.md`, "one additional `_ckpt` GET plus one token-CAS per committed chunk"). The two ref transactions of one part (precommit + commit) are serial phases and can never share a chunk, so a **lone INSERT pays +4 serial RTTs** (2 GETs + 2 CASes); the pre-INV-1 id allocator was an in-memory `fetch_add`, i.e. zero S3, so nothing offsets this. Batching amortizes across concurrent parts only, not across the two phases — the cost concentrates on low-concurrency insert latency. Proposal, protocol-neutral (durability order `log PUT → _ckpt CAS → ack` and all invariants unchanged; only *when the read happens* moves): (a) cache `{body, token}` from the writer's own last winning `_ckpt` CAS in `RefTableRuntime` and let `publishCkpt`'s FIRST attempt go straight to the conditional write — the writer of a namespace is almost always the last author; a stale token simply loses the CAS and falls into the existing whole-body re-read + re-merge retry path (the "read the WHOLE body every attempt" rule in `Pool/CasRefCkpt.cpp` targets retry-after-conflict, which stays intact); the `lifeEpochWouldDecrease` corruption check defers at most one round, to the fresh re-read — a stale token cannot win against a changed object; cold seed (first chunk after open/recovery/wedge) keeps today's GET; (b) optionally overlap that seed GET with the log `PUT` (independent keys; freshness only matters for the retry arm). Effect: +2 → +1 RTT per chunk, +4 → +2 per lone insert; the remaining CAS is irreducible (it IS the frontier mechanism, `NoAckedLoss`). MEASUREMENT PRECONDITION: the stage-1 figure **1.59x predates `_ckpt`** (measured 2026-07-24; `_ckpt` landed 2026-07-28) — re-run the wide-insert baseline on current HEAD first, then bench this change against it. Per the protocol-optimization rule ([[feedback_head_before_put_protocol_untouchable]]): this removes no protocol step and reorders nothing in the durability chain, but it touches the commit path, so it ships only as an explicit reviewed decision with the before/after numbers, not as a drive-by.
