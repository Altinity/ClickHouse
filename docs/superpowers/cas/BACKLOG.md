...
- PHASE-3 VOCABULARY-SETTLING (wire-keys, from the phase-2 final review — decide before phase 3 pins the vocabulary harder): (RC1) the SAME GC-leader identity is wire `lease_owner` in `cas_gc_state` and `owner` in `cas_gc_hb`, which is exactly the split the spec's own `condemn_round` rationale forbids ("two wire spellings would defeat cross-format greppability"); inside `cas_gc_hb` the context rule was applied to `by`→`owner` but not to `seq`→`hb_seq`, while `cas_mount_lease` kept a bare `seq` under that same rule. (RC3) the namespace has three spellings — `namespace` (ref log/snap), `ns` (catalog, spec'd), `root_namespace` (part manifest, where a manifest carries exactly one namespace so `root_` is redundant by the spec's own context rule). (M7) the C++ identifier `ns` names three different wire keys across three codecs, including `PartManifestWire::ns` holding "root_namespace". All three are spec-level naming decisions, not implementation deviations.
