...
(revision 25, 2026-09-04), which supersedes the fix sketch below where they differ.
...
(revision 25, 2026-09-04, after fifteen codex and six opus review rounds; the sixth opus round judged it ready to plan from, the fifteenth codex round restored two rules the rev.21 rewrite had lost, and rev.25 took the owner's three decisions: `Generation` spacing inside the lane, the 200 ms conflict-pause overshoot accepted, idle lanes erased; the lane's `Decide` is the engine's `DecideOnObject`, so `_ckpt` is a one-call change once its `decide` is audited): a per-pool write lane
