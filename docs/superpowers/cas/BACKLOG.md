...
- **[RELINK-CONFIRM-BUSY-LANE] fetch-by-relink availability under sustained writes** — {#relink-confirm-busy-lane} — SUPERSEDED by the 2026-07-30 ref-lane redesign (`bb4dd513118`, the six-state `RefLaneState` contract) — the original per-ref RE-OFFER design this item specced predates it. Current relink fallback paths already log at `INFO`, not `ERROR` (`DataPartsExchange.cpp:921,929,949,1419,1432`), matching this item's severity-demotion ask. Original finding, for context: a busy writer's ref lane is rarely quiescent, so `confirmExactRef` answers `Unknown` often, and the receiver falls back to a byte fetch — a cost/noise issue, not a correctness one (fail-closed, replication converges either way).
