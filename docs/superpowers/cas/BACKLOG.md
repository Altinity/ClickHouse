...
- **[SYSTEM FSCK row omits hard classes]** — MINOR — `SYSTEM CONTENT ADDRESSED FSCK`'s one-row-per-disk result set (`InterpreterSystemQuery.cpp`, `contentAddressedFsckColumns`) carries `dangling` and (since Task 11) `chain_broken`/`unchecked`, but still omits `stale_edge`, `corrupted_runs` and `snapshot_oracle_mismatches` — all three are terms of `FsckReport::clean` and all three make `clickhouse-disks ca-fsck` exit nonzero. A SQL consumer therefore cannot see them at all, which is the same "a finding no run will ever report" shape that hid `corrupted_runs` from the summary line for months (the rule `CasFsck.h`'s `formatFsckSummary` doc states for the text line, never applied to the SQL row). Fix: add the three columns; the row is documented as a deliberate subset, so this is a product-owner call rather than an oversight, but the subset criterion should not be "whichever task last touched it".
