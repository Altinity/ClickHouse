...
- Operator note (Task 25) and lock-task consistency: `system.cas_log`'s `token` column renders `etag:<value>` for `BlobReuseAdopt` and `ManifestPut` while every other event type stays on the bare value until the lock; make one rendering at the lock and document it.
- U1 review prose batch (units/U1-review.md, "Prose"): five FALSE / three IMPRECISE comments around the pool wiring; three are fixed in U1's fix round (`CasPool.h` ×2, `CasRequestBudget.h`), the remaining five go to the docs pass — read the review file for the list.
- SPEC DRIFT (docs pass): §Persisted incarnations says `CasFsck`'s retirement check removes — fsck is read-only and only classifies; §What is promised about requests says "a caller that knows the size may pass it" — `CasOperation::stream` takes no size; the `allocateWriterEpoch` row says it keeps its own switch — it uses `orThrow`, and non-convergence at the deadline is retry-later (`NETWORK_ERROR`), not `CORRUPTED_DATA`, because the bound moved from an attempts cap to a deadline.
- U10 review prose batch (units/U10-review.md P1–P7): the unreachable `Conflict{Meta}` claim; header/report comments asserting `GaveUp` has no count (now false); the under-counting re-sourcing snippet; "the system table renders no Incarnation"; ruling R2's omitted third silent case of `terminate`; the unverified-mutation framing; the `PLANES` doc promising a `Liveness` no production caller passes (U1's `startKeeper` fix makes one).
- SPEC DRIFT (docs pass, from the U9 re-review): the `GaveUp` struct now carries `attempts_sent`; the "carries what" sentence; `CasOperation::pause`/`CasRequests::pause` are absent from the spec; the `ensureBlobPresent` inventory row still names the overruled multi-attempt `publish` policy and the pre-create-first marker reconciliation; the generation-capture scope (one `txn_generation` at transaction start, every upload task on `resume`).
...
- Operator note (Task 25): the mount renewal audit event (`watermark_renew`) no longer emits a per-attempt `retrying` transition nor the keys `unresolved_reason`, `deadline_source`, `stop_cause`; the terminal event carries `classification` (which distinguishes external-lease vs request deadline, unresolved, fence/lifecycle lost) and `attempts_sent`. Document the old→new mapping.
- Engine note (docs pass / possible follow-up): `isDefinitelyRefusedWrite`'s body is under `#if USE_AWS_S3`, so `Refused` is unreachable on a non-S3 build (its counters read zero there); `Declined` carries no `attempts_sent` although a `readModifyWrite` that declines may have spent inner attempts — widen if a consumer ever needs it.
...
- From the CP4′ fixer (2026-09-03): `~Pool`'s phase 1 swallows a failed worker stop and then reaches `CasMountRuntime`'s destructor, which aborts on a joinable thread (correct: a runtime outliving its workers is a programming error); a re-attempted join in `~Pool` phase 1 is the defensible belt-and-braces, a forgiving runtime destructor is not. Docs/tests calibration: the request engine reserves the BACKEND's `attemptTimeoutMs()` per attempt (zero on `InMemoryBackend`), not `CasRequestBudget::attempt_timeout_ms` — a test that derives a clock instant from the budget's attempt timeout is calibrated to the wrong number.
...
- SPEC DRIFT (`docs/superpowers/specs/2026-09-02-cas-backend-token-contract-design.md`, update in the docs task): (1) the `Why::Unresolved` `last_seen` enumeration under-covers — `Object` is reachable (an ambiguous `replace` whose resolve shows the precondition unchanged); (2) "byte comparison, and only then a reissue" states the order the engine no longer uses for `replace` — the unchanged precondition is checked first, byte equality is a proof only once the precondition moved; (3) "the verdict is the call's … of the same call" — the engine scopes ambiguity to the inner write of a `readModifyWrite`, the other fields stay call-wide.
