...
- **[ROUND-DURATION-ALARM] a round running for tens of minutes emits nothing** — {#round-duration-alarm} — SMALL (gc-audit 2026-07-29) — the 41.5-minute never-completing round was invisible because the advisory lease heartbeat kept pulsing and `CasGcScheduler` logs only round OUTCOMES; nothing warns about a round in progress too long. Sharpened by the audit revision: the GC LOG is STRUCTURALLY unable to report the pathology it exists to report — `runRoundLogged` emits Start+Finish rows only, so a never-ending round writes no Finish and the table shows nothing wrong. Fix = periodic in-round PROGRESS ROW in `system.content_addressed_garbage_collection_log` (phase + elapsed + logs folded) plus a WARN once elapsed exceeds a sane bound; observability only, no control-flow.
