...
- **Interpretation:** the dominant cost is content-addressed existence/dedup/reachability HEAD verification on blobs+trees, mostly for absent objects. This loads ANY backend and is the #1 fix target. Exact caller(s) to be traced in the design phase (candidates: `observeAndAdmit`/reachability verification on adopt/merge; read-path existence probes on in-manifest/inline entries).
