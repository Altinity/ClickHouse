...
### Hour-2 (t~128min): disk-growth driver = unreferenced-blob churn vs B160-contended GC
Correctness clean (0 no-ref/broken), responsive, #4 holding (81×503, GC 27f/16rc). roots/ DOWN to 11G (continuous reaper working). **blobs/ = 47G physical but only 2.11 GiB referenced (active parts) → ~22× amplification**: the soak churn (TTL/mutations/truncate) creates unreferenced blobs faster than the B160-contended CA GC reclaims (deleted 38k / candidates 94k). `MAX_POOL_GB=25` is referenced-bytes (2G≪25G → no throttle). blobs/ grows ~20G/h. Disk 294G avail; 12h projection ~251G → fits with ~43G margin IF linear; risk = GC falling further behind (run#5 pattern). **MUST NOT manually reap blobs/** (real CA objects — only the CA GC may delete them; reaper is roots/-only). Lever = monitor; **intervention threshold: if disk used% ≥ 90% (avail < ~180G), stop the workload gracefully** (it will have validated many hours by then). This blobs/ accumulation is the expected consequence of deferring #2 (GC, B160).

### Run #7 (fixed driver) — relaunched 2026-06-16 ~04:08, seed 20260616, 12h
- **Baseline (t=55s):** 0 no-ref/0 broken both, GC 0f/12ok/0rc, 0×412/503/bp, replLag 0, roots 2.1M/blobs 477M, disk 373G. Driver B162 fix in place; reaper armed (continuous). Fixed a stale healthcheck field (reaper log was pinned to soak6 → now newest `*_reaper.log`).
- **Hour-1 (t~61min):** correctness clean (0 no-ref/0 broken, both 0.01s); GC 18f/74ok/10rc (~21% retire-contention = B160, expected); 0×412/100×503/0×bp; roots reaper-bounded 16G (vs run#6 34G); blobs 16G. **replLag spiked to 87s in the sample but was TRANSIENT** — direct check ~1min later showed ch2 `queue_size=0, absolute_delay=0` (a momentary merge/burst; healthcheck samples instantaneous max). Disk 373→329G in h1 (ramp-dominated: roots+blobs reaching ~32G steady). Watch the per-hour blobs rate vs the 180G-avail intervention floor; expect a slowdown after ~3.5h when the live set → ~0 (ts anchored to a fixed ~2h window) shifts the workload to insert→immediate-TTL churn.

### Run #6 ENDED ~3h — driver OOM (B162, FIXED). NOT a CA fault.
`PHASE3 FAILED (rc=137)`. Kernel oom-killer (conclusive, no hand-waving): `Out of memory: Killed process 935198 (python3) total-vm:13618192kB anon-rss:12864068kB ... task=python3` @ 03:25:06. The **python `soak.run` driver itself** grew to **~12.86 GiB** in 3h and was OOM-killed; swap was also exhausted (7G/7G) and Zoom co-resident. Disk was NOT full (375G free). The CA cluster was healthy the entire time (0 no-ref/0 broken, both nodes responsive 0.01s, replLag ≤6s) — this is purely a **harness bug**.
**Root cause:** `Model.rows` (the authoritative per-rid oracle, `soak/model.py`) gained ~100 entries per INSERT op and **never evicted TTL-expired rows**. The table sheds rows via `TTL … DELETE` (active parts fell 311→172→80 over the run) but the oracle dict only grew. Because `ts = base_time + (op_id % TS_WINDOW)` is anchored to a fixed ~2h window at start, after the TTL horizon essentially every inserted row is permanently expired, yet all were retained → monotonic growth (~12.8 GiB at 3h, ~43 GiB at the full stream).
**Fix (committed `fac220cf3c9`):** `Model.prune_expired(now)` evicts rows `_expired`/`live_rows` already exclude (safe — `now` monotonic, `ts` fixed, so once-expired-always-expired; can't change any aggregate at this/any later `now`); called from `checkpoint` after the aggregate assertion (workload drained, `now` quiesced — no race). Also stopped storing the unused 256 B `payload` in the model row (only the INSERT SQL emitter needs it, recomputed independently) — it was the bulk of the per-row footprint. Tests: `test_prune_expired_*`, `test_model_row_carries_no_payload`; full driver suite 142 pass. → **relaunch the 12h soak** with the fixed driver.
