...
**B156 FIXED + reviewed (Approved), relaunching soak run #4 (~2026-06-15):** `reuseBlob(body_recreatable)` converts HEAD-absent→ABORTED only for body-recreatable (putBlob'd, tokened-dep) reuses (retry re-writes content → converges); adopted/committed (tokenless) reuses stay fail-loud FILE_DOESNT_EXIST (no INV-NO-LOSS masking). First blanket attempt (cf454abd826) was caught by adversarial review as masking the adopted-blob loss; the discriminator fix (94ff4f7c9cb, via new Build::depIsTokened) was re-reviewed Approved. Residual follow-up: GC condemning an in-flight build's blob after a heartbeat lapse under load (retry recovers; heartbeat-cadence-vs-GC-grace worth a separate look). Server rebuilt. Relaunching the 12h soak (run #4) with the full fix stack (B151 + B153 RYW + B154 fsck-timeout + B155 readonly-retry + B156 reuse-retry).

**B151 soak #4 pulse (~12:35 UTC, ~1h in):** B151 GREEN (0/122 slow, max 0.01s; 0 under-lock publishes both nodes; reuseBlob-vanish=0 so B156 path not yet triggered). CORRECTION: an initial wedge alarm was a FALSE alarm — `pgrep "soak.run"|head` matched `run_24h.sh` (parent shell, normal do_wait on its python child); the REAL soak.run python (471516) is healthy (Threads:9, hrtimer_nanosleep, 6 active sockets, op_id advancing). NEW SIGNAL TO WATCH: replication lag — both nodes absolute_delay ~60min and GROWING (ch1 3511s/ch2 3626s), queue=10, NO_REPLICA_HAS_PART storm (ch1 171k / ch2 624k). Small queue + huge delay + huge NO_REPLICA count ⇒ likely a STUCK replication-queue entry (a part in the ZK log no replica can fetch, retrying NO_REPLICA tightly). Could be a real part/blob-vanish-vs-replication issue OR single-disk-RustFS fetch starvation under 6-worker load. Driver healthy/progressing — letting it run; watching whether this surfaces at a checkpoint (SYNC REPLICA / count-mismatch). CPU low (ch1 48%/ch2 17%/rustfs 11%). No dangling/PHASE3-FAIL yet.

**B157 root cause CORRECTED + FIXED (TDD), commit `e1f69b696e6` (~2026-06-15):** The run #4 broken parts were FIRST mis-attributed to RustFS saturation; the user's `zgrep` of `20260615_17792_17792_0` showed the real path: `relink`-adopt CLEAN → merge read → `getStorageObjects:645` `FILE_DOESNT_EXIST "no ref for …/op_id.bin"` → "Part looks broken" → detach. Reading the code, that message fires when `resolveRouted` → `resolveRef(allow_stale=true)` returns nullopt — the **part ref itself** is missing from the decode (op_id.bin is just the file in hand). ROOT CAUSE (mine): the Pillar B `allow_stale` TTL decode cache has a read-your-writes COHERENCE RACE. `loadShardDecoded` does HEAD→GET→decode→populate; an in-flight reader can GET a manifest predating a concurrent `Build::publish`, then POPULATE the TTL cache with that stale decode AFTER the publish's `mutateShard` invalidation erase (B153) — a later `allow_stale` read serves the poisoned entry and the just-published part looks absent → wrongly broken → replication GET_PART livelock (353,984× postpones). The merge's own read is fine (publish happens-before it on the same thread); the poison comes from a CONCURRENT reader, so it only surfaced under heavy soak churn. FIX (dominates a revert — keeps the op-count win AND closes the race): per-key monotonic `shard_write_seq` bumped in the `mutateShard` invalidation; `loadShardDecoded` captures it before its GET and only populates if unchanged at populate-time (a write during the GET → skip caching). TDD: `CasStoreDecodeTtl.ConcurrentWriteDuringGetDoesNotPoisonStaleEntry` (`GatedGetBackend`: snapshot-at-entry + gated return, root_shards=1) RED→GREEN; `WarmHitWithinTtlSkipsHead` still green (op-count win intact); 246 `Cas*`/`CaWiring*` green except baseline `CasGcLeak.DisplacedUnexpandedTreeBlobsLeak`. SEPARATE/OPEN: (a) broken-part-refetch livelock recovery robustness; (b) RustFS single-disk saturation (real but concurrent, not the cause — broken-pipe storm 66,915/10min, 412 storms, signature-check spam ~1.6M/node) → op-count reduction / more disks / real S3 to run the aggressive 12h clean. NOT data loss (dangling=0; other replica holds a good copy). Soak re-validation of the fix pending (next run should show no `no ref`-driven broken parts).
