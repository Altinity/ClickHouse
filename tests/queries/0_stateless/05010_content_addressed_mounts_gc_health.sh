...
# B3: system.content_addressed_mounts exposes per-disk GC health (is_leader / pending_reclaim /
...
#
# This is a .sh test (not .sql) because `SYSTEM CONTENT ADDRESSED GARBAGE COLLECTION` now returns a
...
SETTINGS disk = disk(
    ...
    name = '05010_content_addressed_mounts_gc_health',
...
${CLICKHOUSE_CLIENT} -q "SYSTEM CONTENT ADDRESSED GARBAGE COLLECTION '05010_content_addressed_mounts_gc_health'" > /dev/null

${CLICKHOUSE_CLIENT} --multiline -q """
SELECT is_leader, wedged_namespace_count
FROM system.content_addressed_mounts
WHERE disk LIKE '%05010_content_addressed_mounts_gc_health%';
