#!/usr/bin/env bash
# Tags: no-fasttest, no-random-settings, no-random-merge-tree-settings, no-encrypted-storage, no-content-addressed-storage
# Tag no-content-addressed-storage: the test uses an Ordinary database, whose BACKUP path goes via
# temporary hard links - not supported on a content_addressed disk (Code 344 SUPPORT_IS_DISABLED;
