#!/usr/bin/env bash
# Tags: no-replicated-database, no-shared-mergetree, no-content-addressed-storage
...
for _ in {1..10}; do
    ...
    res=$(${CLICKHOUSE_CLIENT} --query "SELECT count() FROM system.part_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND database = currentDatabase() AND table = 't_delete_empty_part_rmt' AND event_type = 'MutatePart'")
...
${CLICKHOUSE_CLIENT} --query "
    ...
    WHERE event_date >= yesterday() AND event_time >= now() - 600 AND database = currentDatabase() AND table = 't_delete_empty_part_rmt' AND event_type = 'MutatePart'
    ORDER BY part_name;
