...
# clickhouse local --query "select name, default from system.settings order by name format TSV" > 02995_settings_26_3_1.tsv
# clickhouse local --query "select name, value from system.merge_tree_settings order by name format TSV" > 02995_merge_tree_settings_settings_26_3_1.tsv

# Previous Antalya release baseline: 26.3.10.20001.altinityantalya (same format as 02995_settings_26_3_1.tsv). Settings listed there are considered known; keep empty or generate from previous Antalya build.
...
$CLICKHOUSE_LOCAL --query "
    ...
    (
        SELECT * FROM file('${CUR_DIR}/02995_settings_26_3_1.tsv', 'TSV', 'name String, default String')
    ),
    ...
    (
        SELECT * FROM file('${CUR_DIR}/02995_merge_tree_settings_settings_26_3_1.tsv', 'TSV', 'name String, default String')
    ),
    new_settings AS
    (
