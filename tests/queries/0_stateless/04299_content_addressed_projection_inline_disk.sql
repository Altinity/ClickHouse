...
-- ALTER ADD/DROP/MATERIALIZE PROJECTION + DETACH/ATTACH durability on the content_addressed disk. We use
-- the server's default content_addressed storage policy here rather than an inline `disk = disk(...)`
...
-- content_addressed-default test job this plain table lands on a CA disk; on the normal job it lands on
