...
-- ALTER ADD/DROP/MATERIALIZE PROJECTION + DETACH/ATTACH on the content_addressed disk. We use the
-- server's default content_addressed storage policy here rather than an inline `disk = disk(...)`
...
-- the projection ALTER mechanics on the CA disk are identical with the default-disk table.
...
-- The freshly built p_sum projection is present and active on the CA disk (the .tmp_proj -> .proj
-- staging flow, the focus of this test).
-- KNOWN CA GAP (backlog): the pre-existing p_by_b projection is NOT carried forward through the
-- MATERIALIZE mutation on the content_addressed disk — on a plain disk both p_by_b and p_sum stay
-- active here, but on CA only p_sum survives. The projection hardlink-carry-forward through a mutation
-- does not reach the rebuilt part's manifest. This does not corrupt query results (the base table still
-- serves every query) but loses an existing projection; it is tracked separately and is out of scope for
-- the temp-projection (.tmp_proj) work this test covers.
...
SELECT 'projections_after_drop', countDistinct(name) FROM system.projection_parts
WHERE database = currentDatabase() AND table = 't_proj_cas_alter' AND active;

-- Persistence: reload from the CA disk and re-read. `p_by_b` is gone, so the count() query falls back
-- to the base table (force_optimize_projection is off here — that is expected, not an error); the
-- surviving `p_sum` still serves the sum(a) aggregation after the reload.
