...
${CLICKHOUSE_CURL} -sS -H "Accept-Encoding: zstd" "${BASE_URL}/play" | zstd -d | grep -o -F '<link rel="icon" href="data:image/svg+xml;base64,PHN2ZyB'

echo "--- test 9: gzip response is valid and decompressable ---"
${CLICKHOUSE_CURL} -sS -H "Accept-Encoding: gzip" "${BASE_URL}/play" | gzip -d | grep -o -F '<link rel="icon" href="data:image/svg+xml;base64,PHN2ZyB'
