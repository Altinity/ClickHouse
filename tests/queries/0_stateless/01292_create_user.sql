...
WHERE
    query NOT LIKE '%query_log%' AND event_date >= yesterday() AND event_time >= now() - 600 AND current_database = currentDatabase() AND
...
CREATE USER user1 IDENTIFIED WITH jwt BY '1'; -- { clientError SYNTAX_ERROR }
CREATE USER user1 IDENTIFIED WITH jwt;
SHOW CREATE USER user1;
CREATE USER OR REPLACE user1 IDENTIFIED WITH jwt PROCESSOR 'my_processor';
SHOW CREATE USER user1;
CREATE USER OR REPLACE user1 IDENTIFIED WITH jwt CLAIMS '{"role":"admin"}';
SHOW CREATE USER user1;
CREATE USER OR REPLACE user1 IDENTIFIED WITH jwt PROCESSOR 'my_processor' CLAIMS '{"role":"admin"}';
SHOW CREATE USER user1;
CREATE USER OR REPLACE user1 IDENTIFIED WITH jwt CLAIMS '{"role":"admin"}' PROCESSOR 'my_processor';
SHOW CREATE USER user1;
ALTER USER user1 IDENTIFIED WITH jwt PROCESSOR 'other_processor';
SHOW CREATE USER user1;
DROP USER user1;
CREATE USER user1 IDENTIFIED WITH jwt PROCESSOR ''; -- { serverError BAD_ARGUMENTS }
CREATE USER user1 IDENTIFIED WITH jwt CLAIMS 'not-json'; -- { serverError BAD_ARGUMENTS }
CREATE USER user1 IDENTIFIED WITH jwt CLAIMS '[]'; -- { serverError BAD_ARGUMENTS }
