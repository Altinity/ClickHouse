...
ON m.id = j.id
ORDER BY m.id
SETTINGS enable_analyzer = 1, enable_alias_marker = 1, distributed_product_mode = 'local'
FORMAT TSVWithNames;
...
ON m.id = j.id
ORDER BY m.id
SETTINGS enable_analyzer = 1, enable_alias_marker = 1, distributed_product_mode = 'local', serialize_query_plan = 1
FORMAT TSVWithNames;
...
ORDER BY m.id
SETTINGS enable_analyzer = 1, enable_alias_marker = 1, asterisk_include_alias_columns = 1, serialize_query_plan = 1
FORMAT TSVWithNames;
...
ORDER BY m.id
SETTINGS enable_analyzer = 1, enable_alias_marker = 1, asterisk_include_alias_columns = 1, joined_subquery_requires_alias = 0, serialize_query_plan = 1
FORMAT TSVWithNames;
