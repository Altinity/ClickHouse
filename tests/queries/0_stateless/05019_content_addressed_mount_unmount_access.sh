...
#  3) Task 7: the same treatment for the third sibling verb, "SYSTEM CONTENT ADDRESSED FSCK" --
#     denied with zero grants, permitted once granted.
...
# Zero grants: all three verbs are denied before the disk is resolved.
...
SYSTEM CONTENT ADDRESSED FSCK 'no_such_disk'; -- { serverError ACCESS_DENIED }
"""
...
# UNMOUNT-only role: UNMOUNT is allowed (fails later, on the disk-type check); MOUNT/FSCK stay denied.
${CLICKHOUSE_CLIENT} --multiline --user user_test_05019 --password user_test_05019 -q """
...
SYSTEM CONTENT ADDRESSED FSCK default; -- { serverError ACCESS_DENIED }
"""
...
# Granting the new right permits MOUNT too (fails later, on the disk-type check); FSCK still denied.
${CLICKHOUSE_CLIENT} --multiline --user user_test_05019 --password user_test_05019 -q """
...
${CLICKHOUSE_CLIENT} --multiline -q """
GRANT SYSTEM CONTENT ADDRESSED FSCK ON *.* TO user_test_05019;
"""

# Granting the FSCK right permits it too (fails later, on the disk-type check).
${CLICKHOUSE_CLIENT} --multiline --user user_test_05019 --password user_test_05019 -q """
SYSTEM CONTENT ADDRESSED FSCK default; -- { serverError BAD_ARGUMENTS }
"""

# All three verbs require an explicit disk (syntax error).
${CLICKHOUSE_CLIENT} --multiline -q """
SYSTEM CONTENT ADDRESSED UNMOUNT; -- { clientError SYNTAX_ERROR }
SYSTEM CONTENT ADDRESSED MOUNT; -- { clientError SYNTAX_ERROR }
SYSTEM CONTENT ADDRESSED FSCK; -- { clientError SYNTAX_ERROR }
"""
