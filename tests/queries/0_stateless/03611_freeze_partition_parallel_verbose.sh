...
BACKUP="${CLICKHOUSE_DATABASE}"
for max_threads in 1 3; do
  FREEZE_QUERY_PARAMS="FORMAT TSVWithNames SETTINGS alter_partition_verbose_result = 1, max_threads = $max_threads"
  ...
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze FREEZE WITH NAME '${BACKUP}' $FREEZE_QUERY_PARAMS;" \
  ...
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze FREEZE PARTITION '3' WITH NAME '${BACKUP}_single' $FREEZE_QUERY_PARAMS;" \
    | ${CLICKHOUSE_LOCAL} --structure "$ALTER_OUT_STRUCTURE, $FREEZE_OUT_STRUCTURE" \
        --query "SELECT command_type, partition_id, part_name, backup_name FROM table ORDER BY partition_id FORMAT TSVWithNames"
  ...
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze ATTACH PARTITION '3' $FREEZE_QUERY_PARAMS;" \
  ...
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze FREEZE PARTITION '7' WITH NAME '${BACKUP}_7', ATTACH PART '5_6_6_0' $FREEZE_QUERY_PARAMS;" \
  ...
  # Unfreeze partition
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze UNFREEZE PARTITION '7' WITH NAME '${BACKUP}_7' $FREEZE_QUERY_PARAMS;" \
    | ${CLICKHOUSE_LOCAL} --structure "$ALTER_OUT_STRUCTURE, $FREEZE_OUT_STRUCTURE" \
        --query "SELECT command_type, partition_id, part_name, backup_name FROM table ORDER BY partition_id FORMAT TSVWithNames"

  # Freeze partition with old syntax
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze_old_syntax FREEZE PARTITION '202103' WITH NAME '${BACKUP}_old_syntax' $FREEZE_QUERY_PARAMS;" \
    | ${CLICKHOUSE_LOCAL} --structure "$ALTER_OUT_STRUCTURE, $FREEZE_OUT_STRUCTURE" \
        --query "SELECT command_type, partition_id, part_name, backup_name FROM table ORDER BY partition_id FORMAT TSVWithNames"

  # Unfreeze partition with old syntax
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze_old_syntax UNFREEZE PARTITION '202103' WITH NAME '${BACKUP}_old_syntax' $FREEZE_QUERY_PARAMS;" \
    | ${CLICKHOUSE_LOCAL} --structure "$ALTER_OUT_STRUCTURE, $FREEZE_OUT_STRUCTURE" \
        --query "SELECT command_type, partition_id, part_name, backup_name FROM table ORDER BY partition_id FORMAT TSVWithNames"

  # Unfreeze the whole backup with SYSTEM query
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze FREEZE PARTITION '7' WITH NAME '${BACKUP}_7_system'"
  ${CLICKHOUSE_CLIENT} --query "DROP TABLE table_for_freeze"
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze UNFREEZE PARTITION '7' WITH NAME '${BACKUP}_7_system'" 2>/dev/null
  rc=$?
  if [ $rc -eq 0 ]; then
      echo "ALTER query shouldn't unfreeze removed table. Code: $rc"
      exit 1
  fi
  ${CLICKHOUSE_CLIENT} --query "SYSTEM UNFREEZE WITH NAME '${BACKUP}_7_system'" \
    | ${CLICKHOUSE_LOCAL} --structure "$ALTER_OUT_STRUCTURE, $FREEZE_OUT_STRUCTURE" \
        --query "SELECT command_type, partition_id, part_name, backup_name FROM table ORDER BY partition_id"
