...
-- ^ no-fasttest: content_addressed is an object-storage metadata type; keep it off the minimal image.
--   no-shared-merge-tree: this exercises open-source ReplicatedMergeTree on a content_addressed disk.
--   no-replicated-database: the source replica_path is hard-coded per the database, not per the replica.

-- ALTER TABLE ... FETCH PARTITION ... FROM '<zk_path>' on a content_addressed disk: the gate is lifted
