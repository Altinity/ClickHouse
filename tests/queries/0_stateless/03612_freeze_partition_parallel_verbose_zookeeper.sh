...
for max_threads in 1 3; do
  ...
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze_replicated FREEZE WITH NAME 'test_03612' $FREEZE_QUERY_PARAMS;" \
  ...
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze_replicated ATTACH PARTITION '3' $FREEZE_QUERY_PARAMS;" \
  ...
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze_replicated FREEZE PARTITION '7' WITH NAME 'test_03612_single_part_7', ATTACH PART '5_0_0_0' $FREEZE_QUERY_PARAMS;" \
