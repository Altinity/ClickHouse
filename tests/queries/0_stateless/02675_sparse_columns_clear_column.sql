...
SELECT count(), groupArray(message) FROM system.text_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND logger_name LIKE '%' || currentDatabase() || '.t_sparse_columns_clear' || '%' AND level = 'Error';
