...
class Git:
    ...
    def _update_tags(self, suppress_stderr: bool = False) -> None:
        ...
        self.latest_upstream_tag = self.check_tag(self.run("git describe --tags --abbrev=0 --match='*-*'", stderr=stderr))
        self.commits_since_upstream = self._commits_since(self.latest_upstream_tag)
