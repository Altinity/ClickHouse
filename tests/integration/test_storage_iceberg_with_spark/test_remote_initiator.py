...
def test_remote_initiator_after_non_remote(started_cluster_iceberg_with_spark, storage_type):
    ...
    queries = instance.query(f"""
        ...
        """)
    # initial node + 2 subqueries on replicas
    assert queries == "3\n"
...
@pytest.mark.parametrize("storage_type", ["s3"])
def test_remote_initiator_global_join_requires_analyzer(started_cluster_iceberg_with_spark, storage_type):
    instance = started_cluster_iceberg_with_spark.instances["node1"]
    spark = started_cluster_iceberg_with_spark.spark_session
    ...
    assert "object_storage_cluster_join_mode=='global' is not supported without allow_experimental_analyzer=true" in instance.query_and_get_error(f"""
        SELECT *
