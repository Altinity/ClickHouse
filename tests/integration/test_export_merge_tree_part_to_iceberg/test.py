...
def test_export_part_source_more_columns_allowed_with_ignore_extra_setting(cluster, mismatch_mode):
    ...
    export_part(
        ...
        extra_settings=f"export_merge_tree_part_schema_mismatch_mode = '{mismatch_mode}'",
    )
...
def test_export_part_column_count_mismatch_source_fewer_still_rejected_with_ignore_extra_setting(
    ...
    error = node.query_and_get_error(
        ...
        f"SETTINGS allow_experimental_export_merge_tree_part = 1, "
        f"allow_experimental_insert_into_iceberg = 1, "
        f"export_merge_tree_part_schema_mismatch_mode = '{mismatch_mode}'"
    )
    assert expected_error in error, f"Expected {expected_error} with {mismatch_mode}, got: {error!r}"
...
@pytest.mark.parametrize("mismatch_mode", EXTRA_SOURCE_COLUMN_MODES)
def test_export_part_ignore_extra_setting_drops_trailing_alias_column(cluster, mismatch_mode):
    node = cluster.instances["node1"]
    sfx = unique_suffix()
...
@pytest.mark.parametrize("mismatch_mode", EXTRA_SOURCE_COLUMN_MODES)
def test_export_part_ignore_extra_setting_kept_alias_depends_on_dropped_column(cluster, mismatch_mode):
    node = cluster.instances["node1"]
    sfx = unique_suffix()
...
def test_export_part_match_by_name_requires_every_destination_column(cluster):
    ...
    error = node.query_and_get_error(
        ...
        f"SETTINGS allow_experimental_export_merge_tree_part = 1, "
        f"allow_experimental_insert_into_iceberg = 1, "
        f"export_merge_tree_part_schema_mismatch_mode = 'ignore_extra_source_columns_by_name'"
    )
