...
@pytest.mark.parametrize("use_roaring_bitmaps", [0, 1])
def test_position_deletes_out_of_order(started_cluster, use_roaring_bitmaps):
    storage_type = "local"
    instance = started_cluster.instances["node1"]
    spark = started_cluster.spark_session
...
def test_read_constant_columns_optimization(started_cluster, storage_type, run_on_cluster):
    ...
    const_partial_expected = instance.query(
        f"SELECT tag, date2, number, name_old FROM {creation_expression} ORDER BY ALL SETTINGS allow_experimental_iceberg_read_optimization=0",
    ...
    const_partial_optimized = instance.query(
        f"SELECT tag, date2, number, name_old FROM {creation_expression} ORDER BY ALL SETTINGS allow_experimental_iceberg_read_optimization=1",
...
"""
Tests for Iceberg format version upgrades performed by external tools.
...
triggering an exception. Each manifest list / manifest file carries its
...
def test_format_version_upgrade_v1_to_v2(started_cluster):
    """Create a v1 table, insert data, upgrade metadata to v2, then read."""
    instance = started_cluster.instances["node1"]
    table_name = f"test_fmt_upgrade_v1_to_v2_{get_uuid_str()}"

    create_iceberg_table(
        "local", instance, table_name, started_cluster, "(x Int)", format_version=1
    )
...
def test_format_version_upgrade_concurrent_reads(started_cluster):
...
@pytest.mark.parametrize("format_version", [1, 2])
def test_format_version_avro_metadata_fallback(started_cluster, format_version, tmp_path):
    ...
    means v2, absence means v1). This test covers both branches of that fallback.
    """
    instance = started_cluster.instances["node1"]
    table_name = f"test_fmt_avro_fallback_v{format_version}_{get_uuid_str()}"

    create_iceberg_table(
        "local", instance, table_name, started_cluster, "(x Int)",
        format_version=format_version,
    )
