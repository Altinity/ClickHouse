...
def test_native_create_gzip_metadata(started_cluster):
    ...
    # engine path does, otherwise the same CREATE TABLE query would produce differently compressed
    # metadata depending on which create path served it.
    node = started_cluster.instances["node1"]
    ...
    # The file is named with the compression token, exactly as `IcebergMetadata::createInitial` names it.
    assert metadata_location.endswith(".gzip.metadata.json"), metadata_location
...
def test_create_table_engine_backend_mismatch_rejected(started_cluster):
    ...
    # Positive control: the matching S3 engine is accepted.
    create_clickhouse_glue_table(
        started_cluster, node, root_namespace, f"{test_ref}_s3", "(x String)"
    )
    assert (
        node.query(f"SELECT count() FROM {CATALOG_NAME}.`{root_namespace}.{test_ref}_s3`") == "0\n"
    )
...
def test_drop_table(started_cluster):
    ...
    # ClickHouse's Glue drop only removes the catalog entry and does not delete the data files, so a purge
    # drop is rejected and the table is left intact.
    error = node.query_and_get_error(
...
def test_namespace_filter(started_cluster):
    ...
    node.query(f"CREATE TABLE {CATALOG_NAME}.`{namespace_prefix}alpha.{table2_name}` (x String) ENGINE = IcebergS3('http://minio1:9001/warehouse-glue/{namespace_prefix}alpha/a1/{table2_name}/', '{minio_access_key}', '{minio_secret_key}')")
    assert "is filtered by `namespaces` database parameter." in node.query_and_get_error(f"CREATE TABLE {CATALOG_NAME}.`{namespace_prefix}bravo.{table2_name}` (x String) ENGINE = IcebergS3('http://minio1:9001/warehouse-glue/{namespace_prefix}bravo/{table2_name}/', '{minio_access_key}', '{minio_secret_key}')")
