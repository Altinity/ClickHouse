...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(CasIdentityLost, "Counts CAS pools that entered the IdentityLost lifecycle state: the pool sentinels (_pool_meta and the owner anchor) were observed authoritatively absent while the pool prefix still held objects (a live erase in progress). Store-class access fails loud from this point; a low-rate observer keeps probing until the erasure completes or the operator recovers by restart / SYSTEM CONTENT ADDRESSED FORGET.", ValueType::Number) \
    M(CasDataRootVanished, "Counts CAS pools that entered a terminal Vanished lifecycle state (erased / replaced / forgotten): the pool's data root is provably gone or was decommissioned. The disk stays registered but store-class access fails loud with a typed error naming the sub-state; restart re-registers the name.", ValueType::Number) \
