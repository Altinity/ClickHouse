...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(CasBlobPutDedup,   "Number of CAS blob deduplicating PUT requests. Grows when uploads reuse existing content.", ValueType::Number) \
    ...
    M(CasBlobCas,        "Number of CAS blob compare-and-swap requests. Grows with concurrent metadata updates.", ValueType::Number) \
    M(CasBlobCasConflict,"Number of CAS blob compare-and-swap conflicts. A non-zero value indicates concurrent update contention.", ValueType::Number) \
    ...
    M(CasRefWedgeFloorReconciled, "Number of CAS ref-log wedges retired because the durable floor already recorded their transaction as durable-but-never-applied (their adoption install could not record it). Non-zero implies that table is POISONED: its cached view is missing a durable transaction until a recovery re-derives it.", ValueType::Number) \
    ...
    M(CasRefCkptPublished, "Number of CAS ref `_ckpt` checkpoint objects durably updated by a token-CAS. Growth tracks snapshot publication and namespace creation.", ValueType::Number) \
    M(CasRefCkptIdenticalSkip, "Number of CAS ref `_ckpt` updates that needed no write because the merged body already matched the stored one. A high ratio against CasRefCkptPublished is the intended steady state.", ValueType::Number) \
    M(CasRefCkptNotAdvanced, "Number of CAS ref-table snapshot publications whose `_ckpt` checkpoint could not be advanced (fenced out, contended, or failed). The snapshot body is durable but is not yet cleanup-authoritative; a non-zero value means recovery keeps replaying from an older base.", ValueType::Number) \
    M(CasGcClampSuppressedPasses, "Number of CAS GC passes that deferred shard graduation and deletion because reachability was uncertain. A non-zero value indicates GC is being held back.", ValueType::Number) \
    ...
    M(CasManifestPutDedup,   "Number of CAS deduplicating manifest PUT requests. Grows when existing manifests are reused.", ValueType::Number) \
    ...
    M(CasManifestCas,        "Number of CAS manifest compare-and-swap requests. Grows with concurrent manifest updates.", ValueType::Number) \
    M(CasManifestCasConflict,"Number of CAS manifest compare-and-swap conflicts. A non-zero value indicates concurrent update contention.", ValueType::Number) \
    ...
    M(CasRootPutDedup,   "Number of CAS deduplicating root PUT requests. Grows when existing roots are reused.", ValueType::Number) \
    ...
    M(CasRootCas,        "Number of CAS root compare-and-swap requests. Grows with concurrent root updates.", ValueType::Number) \
    M(CasRootCasConflict,"Number of CAS root compare-and-swap conflicts. A non-zero value indicates concurrent update contention.", ValueType::Number) \
    ...
    M(CasGcPutDedup,     "Number of deduplicating CAS GC PUT requests. Growth indicates GC reused existing objects.", ValueType::Number) \
    ...
    M(CasGcCas,          "Number of CAS GC compare-and-swap requests. Grows with GC state updates.", ValueType::Number) \
    ...
    M(CasGcCasConflict,  "Number of CAS GC compare-and-swap conflicts. A non-zero value indicates concurrent GC or metadata updates.", ValueType::Number) \
    ...
    M(CasServerPutDedup, "Number of deduplicating CAS server-object PUT requests. Growth indicates reused server objects.", ValueType::Number) \
    ...
    M(CasServerCas,      "Number of CAS server-object compare-and-swap requests. Grows with server metadata updates.", ValueType::Number) \
    M(CasServerCasConflict,"Number of CAS server-object compare-and-swap conflicts. A non-zero value indicates concurrent updates.", ValueType::Number) \
    ...
    M(CasOtherPutDedup,  "Number of deduplicating CAS other-object PUT requests. Growth indicates reused uncategorized objects.", ValueType::Number) \
    ...
    M(CasOtherCas,       "Number of CAS other-object compare-and-swap requests. Grows with metadata updates.", ValueType::Number) \
    M(CasOtherCasConflict,"Number of CAS other-object compare-and-swap conflicts. A non-zero value indicates concurrent updates.", ValueType::Number) \
    ...
    M(CasMetaCas,          "Number of conditional CAS blob-metadata rewrites, including lost races. Growing values indicate metadata updates or contention.", ValueType::Number) \
    ...
    M(CasGcRefScanDisagreements, "Number of ref-log ids on which the two independent enumerations of the ref prefix taken by a SAMPLED GC round disagreed, below the other enumeration's own maximum id for that namespace. An append cannot produce this shape, so a nonzero value means either the object store answered inconsistently about a durable prefix or a deposed leader deleted ref objects concurrently. This is a store-quality reading only: the round's ref intake reads by exact key and is unaffected, so a nonzero value neither aborts a round nor suppresses any destructive step. Read it against CasGcProbeAPerformed -- it can only ever move on a round the detector actually sampled.", ValueType::Number) \
    M(CasGcProbeADue, "Number of GC rounds on which the sampled ref-prefix store-quality detector was DUE by its deterministic cadence (gc_probe_a_period). Due but not performed is counted by CasGcProbeASkipped; a due round that ran is counted by CasGcProbeAPerformed, and the two always sum to this.", ValueType::Number) \
    M(CasGcProbeAPerformed, "Number of GC rounds on which the sampled ref-prefix store-quality detector actually compared two independent enumerations. Each one costs exactly one extra full LIST of the ref prefix. A zero here over a long run, with CasGcProbeADue nonzero, means every sample failed -- see CasGcProbeASkipped.", ValueType::Number) \
    M(CasGcProbeASkipped, "Number of GC rounds on which the sampled ref-prefix store-quality detector was due but could not run, because its own enumeration of the ref prefix failed. The round itself is unaffected (the detector gates nothing); the value exists so a detector that has silently stopped reporting is distinguishable from a store that has stopped lying.", ValueType::Number) \
    M(CasGcProbeAHolePresent, "Of the ref-log ids CasGcRefScanDisagreements counted, how many were CONFIRMED PRESENT by a HEAD taken at the moment of the disagreement. A nonzero value is the direct observation that one of the two enumerations omitted an object that demonstrably existed: the listing was incomplete. This cannot be reconstructed afterwards -- by the time anyone reads a log, the object has legitimately been deleted either way.", ValueType::Number) \
    M(CasGcProbeAHoleAbsent, "Of the ref-log ids CasGcRefScanDisagreements counted, how many were ABSENT when HEADed at the moment of the disagreement. This is a different defect from CasGcProbeAHolePresent and points the opposite way: the OTHER enumeration returned a key that is not there, which indicts the listing client or iterator rather than the store's completeness. Both counters are capped at the per-round anomaly row cap.", ValueType::Number) \
    M(CasGcRebuildVirginByEnumeration, "Number of times SYSTEM CONTENT ADDRESSED GC REBUILD concluded FROM ENUMERATION ALONE that a pool had never sealed a baseline, and so carried no durable holds forward. The verdict rests on three pieces of evidence -- a wide LIST of the gc/gen prefix empty, a narrow probe of gc/gen/1/ empty, and gc/state absent -- and on no point read, because the fold seal key needs an attempt component that is a lease sequence number and has no arithmetic successor to probe. An enumeration that hid every seal INCLUDING generation 1 on a lived-in pool would therefore read as virgin here. Expected to be 0 on any pool that has ever completed a GC round; a nonzero value on such a pool means the rebuild was granted a clean slate it could not prove.", ValueType::Number) \
    ...
    M(CasMountReleaseSkippedForeignOccupant, "Counts CAS mount releases that found a FOREIGN occupant in the slot on a runtime that had already observed its own deposition (write fence latched, renewal stopped). This is the EXPECTED end state of a failover: the farewell is skipped, the successor's slot is left byte-for-byte untouched, and teardown continues. Steady non-zero values on a cluster that is not failing over are worth investigating; a value that tracks failovers is normal.", ValueType::Number) \
    ...
    M(CasIdentityLost, "Counts CAS pools that entered the IdentityLost lifecycle state: the pool sentinels (_pool_meta and the owner anchor) were observed authoritatively absent (both KeyAbsent). IdentityLost is a fail-loud TERMINAL state -- store-class access fails loud from this point and this pool's background threads (remount + GC) self-exit; there is no observer. Recover by restart or SYSTEM CONTENT ADDRESSED FORGET; a matching-sentinel restore does not auto-revive.", ValueType::Number) \
    M(CasDataRootVanished, "Counts CAS pools that entered a terminal Vanished lifecycle state (replaced / forgotten): the pool's data root was proven foreign (pool_id mismatch) or was decommissioned by SYSTEM CONTENT ADDRESSED FORGET. The disk stays registered but store-class access fails loud with a typed error naming the sub-state; restart re-registers the name.", ValueType::Number) \
    M(CasDedupCacheHits, "Number of CAS presence-cache lookups that found a known-present hash. Growth indicates cache reuse and more HEAD-first deduplication checks.", ValueType::Number) \
    M(CasDedupCacheMisses, "Number of CAS presence-cache lookups without an entry. Growing values indicate a cold cache or increasing new-content uploads.", ValueType::Number) \
    M(CasRefRecoveryEpochSealed, "Number of CAS ref epoch-seal transactions MINTED by a recovery CAS-walk, one per dead writer epoch it closed. Each seal occupies the exact log key a dying predecessor's in-flight PUT would take, so growth tracks epoch transitions over touched namespaces (including burned, never-written epochs), not anomalies.", ValueType::Number) \
    M(CasRefRecoveryEpochSealAdopted, "Number of dead CAS ref writer epochs a recovery CAS-walk found ALREADY closed by a concurrent recoverer's seal and adopted. A peer that got there first is the designed outcome, not contention to alarm on.", ValueType::Number) \
    M(CasRefRecoveryStragglerAdopted, "Number of straggler ref-log transactions a recovery CAS-walk met at the slot it tried to seal and adopted, re-sealing at the new T+1. Non-zero means writes from a dying epoch were still materializing when recovery ran.", ValueType::Number) \
