...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(CASRefCkptPublished, "Number of CAS ref `_ckpt` checkpoint objects durably updated by a token compare-and-swap. Growth tracks snapshot publication and namespace creation.", ValueType::Number) \
