...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(CASRelinkConfirmRefusedTableNotResident, "Number of CAS fetch-by-relink confirms answered Unknown because this mount holds no resident runtime for the namespace's ref table (never opened here, released under the ref-table cache budget, dropped with the namespace's catalog life, or cleared by a remount). Cache residency, not writer health: the number moves with the cache budget and with which tables this server has touched, so read it next to the other CASRelinkConfirmRefused* counters only to explain an Unknown, never as a fault.", ValueType::Number) \
