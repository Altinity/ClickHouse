...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(CASBlobDeduplicationCacheHit, "Number of CAS blob presence-cache hits. Growing values indicate effective deduplication and cache reuse.", ValueType::Number) \
    ...
    M(CASDeduplicationCacheHits, "Number of CAS presence-cache lookups that found a known-present hash. Growth indicates cache reuse and more HEAD-first deduplication checks.", ValueType::Number) \
    M(CASDeduplicationCacheMisses, "Number of CAS presence-cache lookups without an entry. Growing values indicate a cold cache or increasing new-content uploads.", ValueType::Number) \
