...
Pipe ReadFromMergeTree::readFromPoolParallelReplicas(RangesInDataParts parts_with_range, Names required_columns, PoolSettings pool_settings)
{
    ...
    for (size_t i = 0; i < pool_settings.threads; ++i)
    {
        ...
        auto processor = std::make_unique<MergeTreeSelectProcessor>(pool, std::move(algorithm), query_info.row_level_filter, query_info.prewhere_info, lazily_read_info, actions_settings, reader_settings);
    ...
    PoolSettings pool_settings)
{
    ...
    for (size_t i = 0; i < pool_settings.threads; ++i)
    {
        ...
        auto processor
        = std::make_unique<MergeTreeSelectProcessor>(pool, std::move(algorithm), query_info.row_level_filter, query_info.prewhere_info, lazily_read_info, actions_settings, reader_settings);
    ...
    UInt64 read_limit)
{
    ...
    for (size_t i = 0; i < parts_with_ranges.size(); ++i)
    {
        ...
        auto processor = std::make_unique<MergeTreeSelectProcessor>(
            pool, std::move(algorithm), query_info.row_level_filter, query_info.prewhere_info, lazily_read_info, actions_settings, reader_settings);
