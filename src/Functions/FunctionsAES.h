...
struct KeyHolder
{
    inline StringRef setKey(size_t cipher_key_size, const StringRef & key) const
    {
        if (key.size != cipher_key_size)
            throw DB::Exception("Invalid key size: " + std::to_string(key.size) + " expected " + std::to_string(cipher_key_size),
...
inline void validateIV(const StringRef & iv_value, const size_t cipher_iv_size)
{
    ...
    if ((mode == CipherMode::MySQLCompatibility && iv_value.size != 0 && iv_value.size < cipher_iv_size)
        ...
        throw DB::Exception("Invalid IV size: " + std::to_string(iv_value.size) + " expected " + std::to_string(cipher_iv_size),
                DB::ErrorCodes::BAD_ARGUMENTS);
}
...
private:
    ...
    void executeImplDryRun(Block & block, const ColumnNumbers & /*arguments*/, size_t result, size_t /*input_rows_count*/) override
    {
        ...
        for (size_t r = 0; r < input_rows_count; ++r)
        {
            ...
            if constexpr (mode == CipherMode::RFC5116_AEAD_AES_GCM)
            {
                ...
                if (input_value.size < tag_size)
                    throw Exception("Encrypted data is too short: only " + std::to_string(input_value.size) + " bytes, "
                            "should contain at least " + std::to_string(tag_size) + " bytes of a tag.",
