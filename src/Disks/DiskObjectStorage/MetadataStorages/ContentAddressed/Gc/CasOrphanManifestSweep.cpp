...
NamespaceProtection activeManifestKeys(Pool & store, const RootNamespace & ns,
                                       const std::optional<ShardCoverage> & coverage, LateLogDedup * dedup)
{
                        ...
                        std::vector<String> * warnings, LateLogDedup * dedup)
{
    ...
    try
    {
        protection = activeManifestKeys(store, ns, view.coverage, dedup);
    ...
    LateLogDedup * dedup)
{
    ...
    for (const ListedKey & listed : page.keys)
    {
        ...
        if (inserted)
        {
            ...
            {
                NamespaceProtection protection =
                    activeManifestKeys(store, parsed->ns, view_it->second.coverage, dedup);
