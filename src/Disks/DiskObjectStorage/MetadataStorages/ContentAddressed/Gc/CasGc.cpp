...
{
    GcState gc_state;
    std::optional<Etag> gc_state_etag;
    ...
    /// are correlated by `round_id` and not by the round number a follower never learns.
    {
        ...
        report.acquired_lease = acquireOrRenewLease(gc_state, gc_state_etag, allow_steal);
    ...
    return LeaseContext{std::move(gc_state), std::move(gc_state_etag), next_round};
}
...
void Gc::runHeartbeatFloorPhase(CasOperation & op, const GcState & leased_state, uint64_t new_round, RoundReport & report)
{
    const Layout & layout = store->layout();
...
std::vector<RunRef> Gc::runParentSealReadPhase(const GcState & gc_state)
{
    ...
    /// `gc_state.snap_generation`/`snap_attempt` in-memory. We compare these against the
                                ...
                                RoundReport * progress)
{
    ...
    std::optional<Etag> gc_state_etag = std::move(lease->state_etag);
    ...
    const uint64_t meta_jobs_scheduled_at_round_start = meta_writer->scheduled();
    const uint64_t meta_jobs_completed_at_round_start = meta_writer->completed();
    ...
    runHeartbeatFloorPhase(op, gc_state, new_round, report);
    ...
    FoldResult folded = fold(gc_state, gc_state_etag, report, new_round, *defer_decision.walk_plan, policy, round_work_budget);
    ...
    /// after it, it would always equal `jobs_scheduled` and say nothing.
    {
        ...
        const uint64_t scheduled = meta_writer->scheduled() - meta_jobs_scheduled_at_round_start;
        const uint64_t completed_on_entry = meta_writer->completed() - meta_jobs_completed_at_round_start;
        meta_writer->drain();
        t.metric("jobs_scheduled", scheduled);
        t.metric("jobs_completed_on_entry", completed_on_entry);
        t.metric("jobs_completed", meta_writer->completed() - meta_jobs_completed_at_round_start);
    }
    ...
    WriteResult commit = op.replace(layout.gcStateKey(), encodeGcState(next), *gc_state_etag,
                                    Retry::standard());
    ...
    gc_state_etag = orThrow(std::move(commit), "CAS gc round commit");
    ...
    /// PHASE 14/18 `handoff_reclaim`.
    {
        ...
        for (const RunRef & old_ref : handoff_candidates)
        {
            ...
            if (old_ref.key_generation > gc_state.snap_pruned_through)
