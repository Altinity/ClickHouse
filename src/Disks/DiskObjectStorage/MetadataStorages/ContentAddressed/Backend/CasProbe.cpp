...
{
    const String src_key = probe_prefix + "/src";
    const String dst_key = probe_prefix + "/dst";

    // Best-effort cleanup — runs unconditionally, on every exit path (success, non-enforcing
    // result, or exception), mirroring runCapabilityProbe's cleanup lambda.
    auto cleanup = [&]() noexcept
    {
        for (const auto & key : {src_key, dst_key})
        {
            try
            {
    ...
    try
    {
        // ---- Step 1: write a tiny throwaway source object. ----
        {
            auto buf = object_storage.writeObject(StoredObject(src_key), WriteMode::Rewrite);
            static constexpr char payload[] = "cas-conditional-copy-probe";
            buf->write(payload, sizeof(payload) - 1);
            buf->finalize();
        }

        // ---- Step 2: conditional copy to a FRESH destination -> must be created. ----
        const auto first = object_storage.copyObjectConditional(
            StoredObject(src_key), StoredObject(dst_key), ReadSettings{}, WriteSettings{});
        if (!first.created)
        {
            // The backend refused to create a fresh destination — broken or misbehaving in a
            // different way than "does not enforce the precondition". Either way, fail closed.
            cleanup();
            return false;
        }

        // ---- Step 3: the SAME conditional copy onto the now-existing destination -> must be
        //              REJECTED (created == false). A backend that reports created == true here
        //              silently overwrote the destination — it does NOT enforce If-None-Match, and
        //              the S3-native staging promote path would be unsafe on it (see CasProbe.h). ----
        const auto second = object_storage.copyObjectConditional(
            StoredObject(src_key), StoredObject(dst_key), ReadSettings{}, WriteSettings{});
    ...
    {
        // Any exception (including the default NOT_IMPLEMENTED thrown by a backend that does not
        // override copyObjectConditional) is fail-closed: conditional copy is NOT supported.
        cleanup();
        return false;
    }
}
