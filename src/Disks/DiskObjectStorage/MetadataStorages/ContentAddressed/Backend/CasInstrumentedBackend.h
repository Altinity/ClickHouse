...
///   publish                    → Put
///
/// `Cas` and `GetStream` are counted by nothing: one conditional write is one request whatever the
/// caller once called it, and a stream open is no longer counted at all. Both enumerators, and the
/// `CAS*CompareSwap`/`CAS*GetStream` events they name, are retired together with the rest of this
/// table when the legacy surface goes.
...
public:
    ...
    /// Uncounted: a stream open is one request, but its event is retired with the legacy surface.
