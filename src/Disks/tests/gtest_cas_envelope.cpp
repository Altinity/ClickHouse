...
/// The v3 blob-envelope shape (512-byte JSON header + payload). Full round-trip / gate / pad-zone /
...
TEST(CASEnvelope, BlobRoundTripNoExtensions)
{
    ...
    h.build_id = 0x33;
    const std::string obj = encodeEnvelopeHeader(h, 512) + payload;
...
TEST(CASEnvelope, IncarnationZoneDoesNotAffectPayload)
{
    ...
    const std::string ha = encodeEnvelopeHeader(a, 512);
    const std::string hb = encodeEnvelopeHeader(b, 512);
