...
TEST(CasGcAckFloor, RecreatedBlobDeleteIsTokenMismatchOk)
{
    ...
    store->renewWatermarkOnce();
    dropRefTransition(*backend, store->layout(), ns, "tbl", r);
    runAuthoritativeRound(gc);   // condemn (captures the ORIGINAL token)
    store->renewWatermarkOnce();
...
TEST(CasGcAckFloor, ResumeAfterCrashBetweenRetiredPutAndStateCas)
{
    ...
    Gc gc2(store, kGc);
    const RoundReport rep = runAuthoritativeRound(gc2);
    store->renewWatermarkOnce();
...
TEST(CasGcCondemnMarker, SwallowedMarkerWriteCarriesEntryInsteadOfDeleting)
{
    ...
    runAuthoritativeRound(gc);   // +1 folds; blob referenced
    dropRefTransition(*backend, store->layout(), ns, "tbl", r);
    runAuthoritativeRound(gc);   // the condemning round; the controlled marker write exhausts as Unresolved
...
TEST(CasGcCondemnMarker, DurableMarkerKeepsCanonicalGraduationSchedule)
{
    ...
    {
        const RoundReport rep = runAuthoritativeRound(gc);   // condemn round K
    ...
    {
        const RoundReport rep = runAuthoritativeRound(gc);   // K+1: confirmed marker => graduates on schedule
        EXPECT_EQ(rep.graduated, 1u);
        const auto e = currentEntryFor(*backend, store->layout(), blob);
        ASSERT_TRUE(e.has_value());
        EXPECT_TRUE(e->delete_pending);
    ...
    {
        const RoundReport rep = runAuthoritativeRound(gc);   // K+2: the pending delete executes
...
TEST(CasGcCondemnMarker, LoadMetaFallbackConfirmsGraduationAfterLeaderRestart)
{
...

    {
        ...
        runAuthoritativeRound(gc);
        dropRefTransition(*backend, store->layout(), ns, "tbl", r);
        const RoundReport rep = runAuthoritativeRound(gc);   // condemn round
        EXPECT_EQ(rep.condemned, 1u);
        store->renewWatermarkOnce();
    }
    const auto lm = loadMetaForTest(*backend, store->layout(), blob);
