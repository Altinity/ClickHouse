...
TEST(CasGcArithmeticIntake, CorruptBodyClampsOneNamespaceWhileAnotherFolds)
{
    ...
    backend->putIfAbsent(layout.refLogKey(ns_a, RefTxnId{1, 2}), "this is not a cas_ref_log object");
...
TEST(CasGcArithmeticIntake, B1IdentityHoldsOverAHoleyCutThatCrossesASeal)
{
    ...
    backend->hide(layout.refLogKey(ns, RefTxnId{1, 2}));
...
TEST(CasGcArithmeticIntake, WhollyOmittedNamespaceFoldsCorrectlyOnceHintReturns)
{
    ...
    for (uint64_t i = 1; i <= 3; ++i)
        publishAt(*backend, layout, ns, RefTxnId{1, i}, "ref_" + std::to_string(i), i,
                  DB::UInt128(i), /*birth=*/i == 1);
    for (uint64_t i = 1; i <= 3; ++i)
        backend->hide(layout.refLogKey(ns, RefTxnId{1, i}));
