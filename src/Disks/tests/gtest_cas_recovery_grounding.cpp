...
TEST(CASRecoveryGrounding, SameEpochFrontierAfterDecodedEpochSealIsCorruption)
{
    ...
    String malformed_ckpt = encodeRefCkpt(RefCkpt{
        ...
        .checkpoint_snapshot_id = std::nullopt,
        .last_epoch_seal = RefTxnId{1, 2}});
    const size_t frontier_sequence = malformed_ckpt.find(R"("committed_through_ref_sequence":"2")");
    ASSERT_NE(frontier_sequence, String::npos);
    malformed_ckpt.replace(frontier_sequence, String{R"("committed_through_ref_sequence":"2")"}.size(), R"("committed_through_ref_sequence":"3")");
...
TEST(CASRecoveryGrounding, TerminalGapBelowFrontierIsCorruptionNotARebirth)
{
    ...
    String malformed_ckpt = encodeRefCkpt(RefCkpt{
        ...
        .checkpoint_snapshot_id = std::nullopt,
        .last_epoch_seal = std::nullopt});
    const size_t frontier_epoch = malformed_ckpt.find(R"("committed_through_writer_epoch":"1")");
    ASSERT_NE(frontier_epoch, String::npos);
    malformed_ckpt.replace(frontier_epoch, String{R"("committed_through_writer_epoch":"1")"}.size(), R"("committed_through_writer_epoch":"2")");
    ASSERT_EQ(backend->putIfAbsent(layout.refCkptKey(life), malformed_ckpt).outcome, PutOutcome::Done);
