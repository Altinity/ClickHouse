...
AccessRights ContextAccess::addImplicitAccessRights(const AccessRights & access, const AccessControl & access_control)
{
    ...
    if (access_control.doesSelectFromSystemDatabaseRequireGrant())
    {
        ...
        /// Per-column tier split is enforced inside fillData; granting
        /// SELECT here is safe.
        if (max_flags.contains(AccessType::SHOW_NAMED_SCALARS) || max_flags.contains(AccessType::getNamedScalar))
            res.grant(AccessType::SELECT, DatabaseCatalog::SYSTEM_DATABASE, "named_scalars");
    }
    else
    {
...
bool ContextAccess::checkAccessImplHelper(const ContextPtr & context, AccessFlags flags, const Args &... args) const
{
    ...
    struct PrecalculatedFlags
    {
        ...
        const AccessFlags named_scalar_ddl = AccessType::CREATE_NAMED_SCALAR | AccessType::DROP_NAMED_SCALAR;
        ...
        const AccessFlags not_readonly_flags = write_table_access | table_and_dictionary_and_function_ddl | workload_ddl | resource_ddl | named_scalar_ddl | write_dcl_access | AccessType::SYSTEM | AccessType::KILL_QUERY;
        const AccessFlags not_readonly_1_flags = AccessType::CREATE_TEMPORARY_TABLE;

        const AccessFlags ddl_flags = table_ddl | dictionary_ddl | function_ddl | workload_ddl | resource_ddl | named_scalar_ddl;
