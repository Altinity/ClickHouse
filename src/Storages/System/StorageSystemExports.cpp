...
ColumnsDescription StorageSystemExports::getColumnsDescription()
{
    return ColumnsDescription
    {
        ...
        {"transaction_id", std::make_shared<DataTypeString>(), "The ID of the export transaction."},
        {"create_time", std::make_shared<DataTypeDateTime>(), "Date and time when the export command was submitted for execution."},
        {"parts_to_do_names", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()), "An array of names of data parts that need to be exported for the transaction to complete."},
        {"parts_to_do", std::make_shared<DataTypeInt64>(), "The number of data parts that need to be exported for the transaction to complete."},
        {"status", std::make_shared<DataTypeString>(), "The status of the export transaction."},
        // {"latest_failed_part_name", std::make_shared<DataTypeString>(), "The name of the most recent part that could not be exported."},
        // {"latest_fail_time", std::make_shared<DataTypeDateTime>(), "The date and time of the most recent part export failure."},
        // {"latest_fail_reason", std::make_shared<DataTypeString>(), "The exception message that caused the most recent part export failure."},
    };
}
...
void StorageSystemExports::fillData(MutableColumns & res_columns, ContextPtr context, const ActionsDAG::Node * predicate, std::vector<UInt8>) const
{
    ...
    for (size_t i_storage = 0; i_storage < col_source_database->size(); ++i_storage)
    {
        ...
        {
            /// fill based on getColumnsDescription
            ...
            res_columns[col_num++]->insert(status.transaction_id);
            res_columns[col_num++]->insert(status.create_time);
            res_columns[col_num++]->insert(parts_to_do_names);
            res_columns[col_num++]->insert(status.parts_to_do_names.size());
            res_columns[col_num++]->insert(String(magic_enum::enum_name(status.status)));
        }
    }
}
