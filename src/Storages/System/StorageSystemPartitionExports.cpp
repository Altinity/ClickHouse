...
ColumnsDescription StorageSystemPartitionExports::getColumnsDescription()
{
    ...
    return ColumnsDescription
    {
        ...
        {"committed_marker_file", std::make_shared<DataTypeString>(),
            "For plain object storage destinations: path of the per-transaction commit marker file written by the destination. Empty for Iceberg destinations and for tasks that have not committed yet. Always empty for plain (non-replicated) MergeTree, which does not persist commit paths."},
...
void StorageSystemPartitionExports::fillData(MutableColumns & res_columns, ContextPtr context, const ActionsDAG::Node * predicate, std::vector<UInt8>) const
{
    ...
    for (const auto & db : DatabaseCatalog::instance().getDatabases(GetDatabasesOptions{.with_datalake_catalogs = false, .with_remote_databases = false}))
    {
        /// skip data lakes
        ...
        for (auto iterator = db.second->getTablesIterator(context); iterator->isValid(); iterator->next())
        {
            ...
            if (!dynamic_cast<MergeTreeData *>(table.get()))
                continue;
