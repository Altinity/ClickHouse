...
void StorageMergeTree::exportPartitionToTable(const PartitionCommand & command, ContextPtr query_context)
{
    ...
    /// partition key / Iceberg partition spec) and capture the destination metadata.json (empty for
    /// non-data-lake destinations).
    ...
    {
        descriptor.iceberg_metadata_json = destination_iceberg_metadata_json;
