...
String getBestPartitionToOptimizeEntire(
    ...
    const LoggerPtr & log)
{
    if (!settings->min_age_to_force_merge_on_partition_only)
        return {};
    ...
    if (occupied > 1 && max_tasks_count - occupied < settings->number_of_free_entries_in_pool_to_execute_optimize_entire_partition)
    {
        LOG_INFO(log,
    ...
    const auto is_partition_invalid = [&](const PartitionStatistics & partition)
    {
        ...
        if (!max_total_size_to_merge || !settings->enable_max_bytes_limit_for_min_age_to_force_merge)
            return false;
    ...
    if (best_partition_min_age < settings->min_age_to_force_merge_seconds || is_partition_invalid(best_partition_it->second))
        return {};
...
void MergeTreeDataMergerMutator::updateTTLMergeTimes(const MergeSelectorChoice & merge_choice, const MergeTreeSettingsPtr & settings, time_t current_time)
{
    ...
    switch (merge_choice.merge_type)
    {
        ...
        case MergeType::TTLDelete:
            next_delete_ttl_merge_times_by_partition[partition_id] = current_time + settings->merge_with_ttl_timeout;
            return;
