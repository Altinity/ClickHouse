...
bool StorageObjectStorage::Configuration::updateIfRequired(
    ObjectStoragePtr object_storage_ptr,
...
{
    if (!updated)
        update(object_storage_ptr, local_context, if_not_updated_before, check_consistent_with_previous_metadata);
    return true;
}
