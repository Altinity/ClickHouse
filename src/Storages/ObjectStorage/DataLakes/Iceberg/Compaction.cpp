    ...
    const StorageID & table_id)
{
    ...
    auto current_manifest_list = getManifestList(
        object_storage, persistent_table_components, context, IcebergPathFromMetadata::deserialize(current_manifest_list_path), log, secondary_storages);
    ...
    Plan & plan, const IcebergPathResolver & path_resolver, ObjectStoragePtr object_storage, SecondaryStorages & secondary_storages, ContextPtr context, SharedHeader sample_block_, String write_format, String table_path)
{
    ...
    for (const auto & history_record : plan.history)
    {
        ...
        auto new_snapshot = metadata_generator.generateNextMetadata(
            plan.generator,
            Iceberg::IcebergPathFromMetadata{},
