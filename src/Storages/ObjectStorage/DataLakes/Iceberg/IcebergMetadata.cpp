    ...
    const std::optional<String> & table_uuid)
{
    ...
    {
        throw Exception(
            ErrorCodes::FILE_DOESNT_EXIST, "The metadata file for Iceberg table with path {} doesn't exist", configuration_ptr->getRawPath().path);
    }
    ...
    Poco::Logger * log)
{
    ...
    if (data_lake_settings[DataLakeStorageSetting::iceberg_metadata_file_path].changed)
    {
        ...
        try
        {
            ...
            auto prefix_storage_path = configuration_ptr->getRawPath().path;
...
void IcebergMetadata::updateState(const ContextPtr & local_context, Poco::JSON::Object::Ptr metadata_object, bool metadata_file_changed)
{
    ...
    if (timestamp_changed)
    {
        ...
        if (!metadata_object->has(f_snapshot_log))
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "No snapshot log found in metadata for iceberg table {} so it is impossible to get relevant snapshot id using timestamp", configuration_ptr->getRawPath().path);
...
ManifestFileCacheKeys IcebergMetadata::getManifestList(ContextPtr local_context, const String & filename) const
{
    ...
    auto create_fn = [&]()
    {
        ...
        for (size_t i = 0; i < manifest_list_deserializer.rows(); ++i)
        {
            ...
            const auto manifest_file_name = getProperFilePathFromMetadataInfo(file_path, configuration_ptr->getRawPath().path, table_location);
