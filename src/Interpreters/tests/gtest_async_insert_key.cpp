...
TEST(AsyncInsertKey, ForwardedTokenPartitionsBatches)
{
    ...
    /// Queue keys are copied when scheduling flushes; preserve the credential and batching key.
    auto copied = original;
    EXPECT_EQ(copied.forwarded_auth_token, token);
    EXPECT_EQ(copied, original);
    EXPECT_EQ(copied.hash, original.hash);
    EXPECT_NE(copied, rotated);

    rotated = original;
    EXPECT_EQ(rotated.forwarded_auth_token, token);
    EXPECT_EQ(rotated, original);
    EXPECT_EQ(rotated.hash, original.hash);

    token.reset();
    EXPECT_EQ(copied.forwarded_auth_token->token, "alice-token");
}
