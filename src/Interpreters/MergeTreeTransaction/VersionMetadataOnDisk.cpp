...
std::expected<Int32, StaleVersion> VersionMetadataOnDisk::storeInfoUnlocked(VersionInfo new_info)
{
    ...
    if (!can_write_metadata)
    {
        if (involved_in_transaction)
        {
            if (merge_tree_data_part->isStoredOnReadonlyDisk())
                throw Exception(
                    ErrorCodes::NOT_IMPLEMENTED, "Object {} was involved in transaction but cannot write metadata", getObjectName());
    ...
    if (expected_storing_version != new_info.storing_version)
    {
        LOG_DEBUG(
            log,
            "Object {}, mismatched storing version {}, expected {}",
            getObjectName(),
            new_info.storing_version,
            expected_storing_version);
        return std::unexpected(TOO_OLD_VERSION);
    }
...
std::expected<Int32, StaleVersion> VersionMetadataOnDisk::storeInfo(const VersionInfo & new_info)
{
    std::lock_guard lock_storing_version{persisted_info_mutex};
    return storeInfoUnlocked(new_info);
}
...
void VersionMetadataOnDisk::removeTmpMetadataFile()
{
    ...
    if (!merge_tree_data_part->isStoredOnReadonlyDisk())
        ...
        LOG_WARNING(log, "Object {}, disk is readonly, will not remove {}", getObjectName(), TMP_TXN_VERSION_METADATA_FILE_NAME);
}
