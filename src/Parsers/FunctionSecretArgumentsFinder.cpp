...
void FunctionSecretArgumentsFinder::markSecretArgument(size_t index, bool argument_is_named)
{
    ...
    /// The index is translated back to the raw argument index: `index` addresses the skip-adjusted
    /// view (see `Arguments::skipArgument`), but the result is consumed against the real argument list.
    auto [it, inserted] = result.masked_arguments.emplace(function->arguments->getRealIndex(index), argument_is_named);
    if (!inserted)
                    ...
                    else if (key == "url")
                    {
                        ...
                        if (f->arguments->at(1)->tryGetString(&url, /* allow_identifier= */ false))
                        {
                            if (maskS3URICredentials(url))
                                result.replaced_arguments[function->arguments->getRealIndex(i)] = "url = " + quoteString(url);
                        }
                        else
                        {
...
void FunctionSecretArgumentsFinder::maskS3UrlArgument(const std::vector<size_t> & positional, size_t url_slot)
{
    ...
    if (maskS3URICredentials(url))
        result.replaced_arguments[function->arguments->getRealIndex(positional[url_slot])] = quoteString(url);
}
...
std::string FunctionSecretArgumentsFinder::findIcebergStorageType()
{
...
void FunctionSecretArgumentsFinder::findIcebergFunctionSecretArguments()
{
...
ssize_t FunctionSecretArgumentsFinder::findNamedArgument(String * res, const std::string_view & key, size_t start)
{
    for (size_t i = start; i < function->arguments->size(); ++i)
    {
...
bool FunctionSecretArgumentsFinder::findSecretNamedArgument(const std::string_view & key, size_t start)
{
