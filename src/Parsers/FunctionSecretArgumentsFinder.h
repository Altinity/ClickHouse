...
protected:
    ...
    /// The `iceberg`/`Iceberg` locators are storage-agnostic: the underlying object storage is chosen
    /// by a `storage_type` argument (or by the same key inside a named collection), and the remaining
    /// arguments follow the S3 or Azure form accordingly. Returns the lowercased storage type,
    /// defaulting to "s3", and skips the `storage_type` argument so the S3/Azure parsers see the
    /// argument list at the positions they expect. The caller must `unskipArguments()` afterwards.
    ...
    /// `url_offset` (1 for `urlCluster`, which puts the cluster name first; 0 otherwise).
    void findURLSecretArguments(size_t url_offset = 0);
