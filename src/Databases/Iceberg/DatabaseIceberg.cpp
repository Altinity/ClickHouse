...
{
    if (settings.warehouse.value.empty())
    {
        throw Exception(
...
std::shared_ptr<Iceberg::ICatalog> DatabaseIceberg::getCatalog() const
{
    ...
    switch (settings.catalog_type.value)
    {
        case DB::DatabaseIcebergCatalogType::REST:
        {
            catalog_impl = std::make_shared<Iceberg::RestCatalog>(
                settings.warehouse.value,
                url,
                settings.catalog_credential.value,
                settings.auth_scope.value,
                settings.auth_header,
                settings.oauth_server_uri.value,
...
{
    auto endpoint_from_settings = settings.storage_endpoint.value;
...
StoragePtr DatabaseIceberg::tryGetTable(const String & name, ContextPtr context_) const
{
    ...
    const bool with_vended_credentials = settings.vended_credentials.value;
...
void registerDatabaseIceberg(DatabaseFactory & factory)
{
    auto create_fn = [](const DatabaseFactory::Arguments & args)
    {
        if (!args.create_query.attach
            && !args.context->getSettingsRef().allow_experimental_database_iceberg)
        {
