...
#define DATABASE_ICEBERG_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, oauth_forward_user_token, false, "Authenticate to the catalog as the user running the query instead of as the shared identity configured on the database. For an Iceberg REST catalog this forwards the user's own bearer token unchanged (passthrough), or exchanges it first when `oauth_token_exchange_uri` is set. For a Glue catalog, which speaks AWS SigV4 rather than bearer tokens, the token is exchanged for temporary AWS credentials with `AssumeRoleWithWebIdentity` against `aws_role_arn`. Requires the server-level `enable_token_forwarding` setting", 0) \
