...
void DatabaseDataLake::validateCreateTableEngineArguments(const ASTFunction & engine) const
{
    ...
    /// `tryGetTableImpl` reopens every table as the database engine arguments with the table location
    /// substituted for the first one, so the only argument this `CREATE` can express is that location.
    /// Any other argument - storage credentials above all - would be used to write the table here and
    /// then be replaced by the database's one on the first read, silently moving the table elsewhere.
    ...
    /// With `virtual_hosted` addressing the reopen path moves the bucket into the hostname, so the
    /// resulting URL no longer starts with `storage_endpoint` and cannot be compared against it.
    ...
    const ASTPtr & query)
{
    ...
    /// trace in the catalog. The namespace's default location must be the namespace base, not this first
    /// table's directory, or later tables created without an explicit location would land under it.
