...
JWTProvider::JWTProvider(
    ...
    std::ostream & out,
    std::ostream & err)
    : oauth_url(std::move(auth_url)),
      oauth_client_id(std::move(client_id)),
      oauth_audience(std::move(audience)),
      output_stream(out),
      error_stream(err)
{
}
...
{
    if (oauth_client_id.empty() || oauth_url.empty())
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Error: --oauth-client-id and --oauth-url are required for --login.");

    Poco::URI device_code_url(oauth_url + "/oauth/device/code");
    Poco::URI token_url(oauth_url + "/oauth/token");
    ...
    std::ostream & err)
{
    ...
    {
        return std::make_unique<CloudJWTProvider>(auth_url, client_id, audience, host, out, err);
    }
    else
    {
        return std::make_unique<JWTProvider>(auth_url, client_id, audience, out, err);
    }
}
