...
function git_clone_with_retry
{
    ...
    for _ in 1 2 3 4; do
        rm -rf "${repo_dir}" ||:
        if (git clone --depth 1 https://github.com/Altinity/ClickHouse.git -- "${repo_dir}" \
            && cd "${repo_dir}" \
            && git fetch --depth 1 origin "${BASE_REF}" \
            && git checkout "${BASE_REF}") 2>&1 \
            | ts '%Y-%m-%d %H:%M:%S'; then
            return 0
        else
            sleep 0.5
...
<h1>AST Fuzzer for PR <a href="https://github.com/Altinity/ClickHouse/pull/${PR_TO_TEST}">#${PR_TO_TEST}</a> @ ${SHA_TO_TEST}</h1>
<p class="links">
